...
The goal is net-negative source and repository bloat without breaking parity. `examples/winstt-electron/` remains the parity reference and is not part of active app cleanup unless the project policy changes.
...
- Manual consolidation of locale JSON, TTS engine implementations, STT engine implementations, and `examples/winstt-electron/`.
...
- Do not touch `examples/winstt-electron/` except for reference reads.
...
- Keep the small compatibility wrappers that normalize fallback values, event payloads, or Electron parity shapes.
