...
<StatGrid>
  <Stat value="$0" label="Free, MIT-licensed" />
  <Stat value="100%" label="Local by default" />
  <Stat value="40+" label="Models, 8 families" />
...
No. Cloud LLM cleanup (OpenRouter) and cloud STT (ElevenLabs, OpenRouter) are strictly opt-in and locked until you add your own API key under [Integrations](/docs/settings/integrations). With no keys configured, WinSTT is 100% local. If you use a cloud provider, you pay that provider directly — WinSTT takes no cut and adds no markup.

## Languages

### What languages can WinSTT transcribe?

That depends on the model you pick. Pin a language under [Settings → Model](/docs/settings/model) or leave it on auto-detect.

<ModelTable
  head={["Model family", "Coverage"]}
  rows={[
  ...
  caption="Transcription coverage is per-model; the interface language is set independently."
/>
