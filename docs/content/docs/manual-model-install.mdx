...
<ModelTable
  ...
  caption="The catalog at server/src/recorder/domain/catalog.json is the authoritative list — the onnx_model_name field holds the exact repo ID for every entry."
/>
</Step>
...
To sanity-check the layout before booting Electron, run the cache probe in isolation from `server/`:

```bash
cd server
uv run python -c "from recorder.infrastructure.model_cache import probe_cache_state; print(probe_cache_state('onnx-community/whisper-tiny.en'))"
```

The state field tells you exactly where you stand:

<ModelTable
  head={["State", "Meaning", "Fix"]}
  rows={[
    ["cached", "All expected weight files are present — the picker will show Cached.", "Nothing — you're done."],
    ["partial", "A file is missing or an .incomplete marker is still present.", "Re-copy the snapshot; check for leftover *.incomplete files."],
    ["not_cached", "The snapshots/<sha>/ folder isn't there at all.", "Recheck the folder name (models--<owner>--<repo>) and the commit SHA in refs/main."],
  ]}
  caption="probe_cache_state returns a ModelCacheState(state=…); cached reports equal downloaded/total bytes."
/>
