...
export function hold(
  ...
  if (frame >= outStart) return 1 - ramp(frame, outStart, outEnd, Easing.in(Easing.cubic));
  return 1;
}
...
export function TypeText({
  ...
  const frame = useCurrentFrame();
  const chars = Math.floor(mapRange(frame, [from, from + duration], [0, text.length], Easing.linear));
...
export function ShimmerText({
  ...
  const frame = useCurrentFrame();
  const x = mapRange(frame, [activeFrom, activeFrom + 70], [-120, 160], inOut);
  return (
    <span
      style={{
        color: "transparent",
        backgroundImage: `linear-gradient(90deg, ${C.muted} 0%, ${C.fg} 48%, ${C.muted} 100%)`,
        backgroundClip: "text",
...
export function Bars({
  ...
  const frame = useCurrentFrame();
  return (
    <div style={{ display: "flex", alignItems: "center", gap: width * 0.62, height }}>
...
export function RadialMeter({ accent = C.accent }: { accent?: string }) {
  ...
  return (
    <svg width="360" height="360" viewBox="0 0 360 360">
      ...
      <text x={cx} y={cy + 6} textAnchor="middle" fill={C.fg} fontSize="18" fontFamily={C.mono}>
        LIVE
      </text>
    </svg>
  );
...
export function ProgressSteps({
  ...
  const node = 38;
  return (
    <div style={{ position: "relative", height: 92, width: gap * (steps.length - 1) + node, ...style }}>
      <div
        style={{
          position: "absolute",
          left: node / 2,
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={step} style={{ position: "absolute", left: i * gap, top: 0, width: node }}>
            <div
              style={{
                width: node,
                height: node,
...
export function MockCursor({
  ...
  const click = clickAt == null ? 0 : hold(frame, clickAt, clickAt + 4, clickAt + 18, clickAt + 28);
  return (
    <div
      style={{
        position: "absolute",
    ...
    >
      ...
      <svg width="28" height="28" viewBox="0 0 28 28">
        <path d="M5 3l16 11-8 2-4 8z" fill={C.fg} stroke={C.black} strokeWidth="1.4" />
      </svg>
    </div>
  );
}
