 ...
 *
 * Usage:
 *   bun tools/i18n/check-i18n.ts            # human report; exits 1 on missing/stale
 *   bun tools/i18n/check-i18n.ts --strict   # also exit 1 if anything is untranslated
 *   bun tools/i18n/check-i18n.ts --verbose  # list every offending key, not just N
 *   bun tools/i18n/check-i18n.ts --json     # machine-readable report to stdout
 *
 * This is the live replacement for the old `check-translations.ts`, which pointed
 * at a deleted legacy locale tree instead of the bundles that actually ship
 * (`messages/*.json`, imported by `src/shared/i18n/messages.ts`).
 */
...
function printList(
	...
	if (!VERBOSE && keys.length > PREVIEW) {
		console.log(
			c(`      … and ${keys.length - PREVIEW} more (use --verbose)`, "gray"),
		);
	}
}
...
function main(): void {
		...
		for (const r of reports) {
			...
			console.log(
				`${mark} ${r.locale.padEnd(5)}` +
					...
					c(
						`${r.missing.length}`.padStart(9),
						r.missing.length ? "red" : "gray",
					) +
	...
	if (JSON_OUT) {
		process.exit(
			structuralProblems > 0 || (STRICT && untranslatedTotal > 0) ? 1 : 0,
		);
	}
	...
	if (structuralProblems === 0) {
		console.log(
			c("✓ Structure: all locales have exact key parity with en.json", "green"),
		);
	} else {
		console.log(
			c(
				`✗ Structure: ${structuralProblems} missing/stale keys across locales`,
				"red",
			),
		);
	}
	if (untranslatedTotal === 0) {
		console.log(
			c("✓ Coverage: every translatable string is localized", "green"),
		);
	} else {
		console.log(
			c(
