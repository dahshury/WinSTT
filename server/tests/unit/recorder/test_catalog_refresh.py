...
def fake_hf(monkeypatch: pytest.MonkeyPatch) -> Iterator[dict[str, Any]]:
    ...
    monkeypatch.setitem(
        __import__("sys").modules, "huggingface_hub", fake_hub
    )
...
class TestLanguageFetcher:
    ...
    def test_fetch_card_languages_unexpected_type_returns_none(
        self, fake_hf: dict[str, Any]
    ) -> None:
...
class TestResolveHfRepo:
    ...
    def test_short_alias_resolves_via_onnx_asr(self, fake_resolver: dict[str, str]) -> None:
        ...
        assert (
            catalog_refresh._resolve_hf_repo("nemo-canary-1b-v2")
...
class TestLanguagesFor:
    def test_english_whisper_pinned_to_en(
        self, fake_hf: Mapping[str, Any], fake_resolver: Mapping[str, str]
    ) -> None:
        info = _info("tiny.en", family="whisper", onnx="onnx-community/whisper-tiny.en")
        assert catalog_refresh._languages_for(info) == ["en"]

    def test_whisper_routes_through_openai_reference(
        self, fake_hf: dict[str, Any]
    ) -> None:
    ...
    def test_lite_whisper_also_uses_openai_reference(
        self, fake_hf: dict[str, Any]
    ) -> None:
    ...
    def test_nemo_canary_uses_resolved_repo(
        self, fake_hf: dict[str, Any], fake_resolver: dict[str, str]
    ) -> None:
        fake_resolver["nemo-canary-1b-v2"] = "istupakov/canary-1b-v2-onnx"
...
class TestFetchLanguageOverlay:
    def test_only_models_with_languages_appear(
        self, fake_hf: dict[str, Any], fake_resolver: dict[str, str]
    ) -> None:
