...
export function ipcClientMock(): Record<string, unknown> {
	return {
		...
		getFilePath: (file: File): string => (hasBridge() ? (api()?.getPathForFile(file) ?? "") : ""),
		...
		copyLastTranscript: () => invokeOrDefault<boolean>(IPC.TRANSCRIPT_COPY_LAST, false),
		...
		onTranscriptionFailed: (cb: () => void) => on(IPC.STT_TRANSCRIPTION_FAILED, cb),
		onSttSessionAborted: (cb: () => void) => on(IPC.STT_SESSION_ABORTED, () => cb()),
		sttAbortOperation: () => send(IPC.STT_ABORT_OPERATION),
		overlaySetIgnoreMouse: (ignore: boolean) => send(IPC.OVERLAY_SET_IGNORE_MOUSE, { ignore }),
		...
		fileQueueCancel: (id: string) => invokeOrDefault<null>(IPC.FILE_QUEUE_CANCEL, null, { id }),
		fileQueueRetry: (id: string) => invokeOrDefault<null>(IPC.FILE_QUEUE_RETRY, null, { id }),
		fileQueueCopy: (id: string) => invokeOrDefault<null>(IPC.FILE_QUEUE_COPY, null, { id }),
		fileQueueClear: () => invokeOrDefault<null>(IPC.FILE_QUEUE_CLEAR, null),
		fileQueuePause: (id: string) => invokeOrDefault<null>(IPC.FILE_QUEUE_PAUSE, null, { id }),
		fileQueueResume: (id: string) => invokeOrDefault<null>(IPC.FILE_QUEUE_RESUME, null, { id }),
		fileQueueDiscardAll: () => invokeOrDefault<null>(IPC.FILE_QUEUE_DISCARD_ALL, null),
		fileQueueGetActive: () => invokeOrDefault<boolean>(IPC.FILE_QUEUE_GET_ACTIVE, false),
		onFileQueueUpdate: (cb: (data: unknown) => void) => onCast(IPC.FILE_QUEUE_UPDATE, cb),
		onFileQueueProgress: (cb: (data: unknown) => void) => onCast(IPC.FILE_QUEUE_PROGRESS, cb),
		onFileQueueActive: (cb: (data: unknown) => void) => onCast(IPC.FILE_QUEUE_ACTIVE, cb),
		...
		ttsListModelsWithState: () => invokeOrDefault<unknown>(IPC.TTS_LIST_MODELS_WITH_STATE, null),
		fetchTtsModelsWithState: () => invokeOrDefault<unknown>(IPC.TTS_LIST_MODELS_WITH_STATE, null),
		...
		ttsCloudListVoices: () =>
			invokeOrDefault<unknown>(IPC.TTS_CLOUD_LIST_VOICES, { voices: [], unavailable: true }),
		ttsCloudPreview: (payload: { previewUrl: string }) =>
			invokeOrDefault<unknown>(IPC.TTS_CLOUD_PREVIEW, { requestId: "" }, payload),
		ttsCloudSubscription: () =>
			invokeOrDefault<unknown>(IPC.TTS_CLOUD_SUBSCRIPTION, { creditsExhausted: false }),
		...
		updaterCheckNow: () => invokeOrDefault<unknown>(IPC.UPDATER_CHECK_NOW, { triggered: false }),
		updaterQuitAndInstall: () =>
			invokeOrDefault<unknown>(IPC.UPDATER_QUIT_AND_INSTALL, { triggered: false }),
	};
}
