...
describe("variantDisplayName", () => {
  ...
  test("strips the redundant parameter-count token from the name", () => {
    expect(variantDisplayName(named("NeMo Canary 180M Flash", "nemo"))).toBe(
      "Canary Flash",
    );
    expect(variantDisplayName(named("NeMo Canary 1B v2", "nemo"))).toBe(
      "Canary v2",
    );
  ...
  test("strips the leading family label (the chip already conveys it)", () => {
    expect(variantDisplayName(named("Whisper Large v3", "whisper"))).toBe(
      "Large v3",
    );
    expect(
      variantDisplayName(named("Lite-Whisper Large v3 Turbo", "lite-whisper")),
    ).toBe("Large v3 Turbo");
  });
  ...
  test("leaves version tokens and product numbers without an M/B suffix intact", () => {
    ...
    expect(variantDisplayName(named("GigaAM v3 E2E CTC", "gigaam"))).toBe(
      "v3 E2E CTC",
    );
    expect(variantDisplayName(named("Breeze ASR 25", "whisper"))).toBe(
      "Breeze ASR 25",
    );
  });

  test("drops language qualifiers while keeping flavour qualifiers", () => {
  ...
  test("falls back to the raw name if stripping would empty it", () => {
    ...
    expect(variantDisplayName(named("Whisper 39M", "whisper"))).toBe(
      "Whisper 39M",
    );
  });
  ...
  test("keeps the size token when dropping it would collide with a peer", () => {
    ...
    const v2 = {
      ...model("nemo-canary-1b-v2", "nemo"),
      displayName: "NeMo Canary 1B v2",
    };
...
describe("groupByFamily", () => {
  ...
  test("sorts models within each family by parameter count ascending", () => {
    ...
    expect(grouped[0]?.[1].map((m) => m.id)).toEqual([
      "w-tiny",
      "w-small",
      "w-medium",
      "w-large",
