...
export interface ModelCardProps {
	...
	favorite?: { isFavorited: boolean; label: string; onToggle: () => void } | undefined;
	...
	return (
		<div className="flex min-w-0 flex-1 flex-col gap-1.5">
			<div className="flex min-w-0 items-center gap-1.5">
				...
				{unavailable ? (
					<UnavailableBadge errorMessage={errorMessage} label={unavailableLabel} />
				) : null}
			</div>
			...
			{!unavailable && description ? (
				<p className="line-clamp-2 text-[11px] text-foreground-muted leading-snug">{description}</p>
			) : null}
...
function RightCluster({ actions, favorite, perf, trailing, unavailable }: RightClusterProps) {
	return (
		<div className="flex shrink-0 items-start gap-3">
			{!unavailable && perf ? (
				<PerfBars accuracyScore={perf.accuracyScore} speedScore={perf.speedScore} />
			) : null}
...
export function ModelCard({
	...
	const body = (
		<>
			...
			{!unavailable && shelf ? <div className={RECESSED_SHELF_CLASSES}>{shelf}</div> : null}
		</>
	);
