...
export function getEngineConfig(engine: TtsEngineKey | string): TtsEngineConfig {
...
const ENGINE_SEARCH_ALIASES: Record<string, string[]> = {
	...
	supertonic: ["supertone", "supertonic-en"],
 ...
 */
export function groupModelsByEngine(models: readonly TtsModelInfo[]): TtsEngineGroup[] {
	...
	groups.sort((a, b) => (a.items[0]?.paramCountM ?? 0) - (b.items[0]?.paramCountM ?? 0));
	return groups;
}
...
export function cloningLabel(cloning: TtsModelInfo["cloning"]): {
	...
	if (cloning === "zero_shot_audio_transcript") {
		return {
			label: "Voice cloning",
			tooltip: "Zero-shot voice cloning from a short reference audio clip and its transcript",
		};
	}
	return null;
}
