...
class ActivateWindowUseCase:
    ...
    config: ActivationConfiguration, window_info: WindowInfo, response: ActivateWindowResponse,
        ...
        try:
            ...
            if config.restore_if_minimized and window_info.state == WindowState.MINIMIZED:
                if not self.window_activator.restore_window(window_info.handle):
                    return False
            ...
            # Bring to foreground
            if config.bring_to_foreground:
                if not self.window_activator.bring_to_foreground(window_info.handle):
                    return False
