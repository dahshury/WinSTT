    ...
    def execute(self, request: ExportSettingsRequest,
        ...
        try:
            self._logger.log_info(
                ...
                export_path=str(request.export_path.value)
                format=request.export_format.value,
            )

            # Phase 1: Initialize and validate
            if not self._update_progress(request.progress_callback, ExportPhase.INITIALIZING, 0):
                response.result = ExportResult.CANCELLED
                return response
            ...
            if not self._update_progress(request.progress_callback, ExportPhase.VALIDATING_SETTINGS, 20):
                response.result = ExportResult.CANCELLED
                return response

settings_validation = (
    self._validation.validate_settings_data(request.settings_to_export))
            if not settings_validation.is_success:
response.error_message = (
    f"Settings validation failed: {settings_validation.error_message}")
                return response

            # Phase 3: Prepare data for export
            if not self._update_progress(request.progress_callback, ExportPhase.PREPARING_DATA, 40):
                response.result = ExportResult.CANCELLED
                return response
...
response.error_message = (
    f"Data preparation failed: {export_data_result.error_message}")
                return response
            ...
            # Phase 5: Write file
            if not self._update_progress(request.progress_callback, ExportPhase.WRITING_FILE, 60):
                response.result = ExportResult.CANCELLED
                return response
            ...
            if request.configuration.verify_export:
                if not self._update_progress(request.progress_callback, ExportPhase.VERIFYING_EXPORT, 80):
                    response.result = ExportResult.CANCELLED
                    return response

verify_result = (
    self._validation.verify_exported_file(request.export_path, export_data))
                if not verify_result.is_success:
                    response.warnings.append(f"Export verification failed: {verify_result.error_mess\
    age}")

            # Phase 7: Complete
            if not self._update_progress(request.progress_callback, ExportPhase.COMPLETED, 100):
                response.result = ExportResult.CANCELLED
                return response

            # Get file size
            size_result = self._file_system.get_file_size(Path(request.export_path.value))
...
path_validation = (
    self._validation.validate_export_path(request.export_path, request.export_format))
    ...
    def _prepare_export_data(self, request: ExportSettingsRequest,
        ...
        try:
            ...
            export_data = {
                "model": settings.model_configuration.model_name,
                "quantization": settings.model_configuration.quantization.value,
                "recording_sound_enabled": settings.sound_configuration.enabled,
                "sound_file_path": str(settings.sound_configuration.file_path.value) if settings.sound_configuration.file_path else None,
                "output_srt": settings.output_configuration.srt_enabled,
                "recording_key": str(settings.hotkey_binding.key_combination.value)
                "llm_enabled": settings.llm_configuration.enabled,
                "llm_model": settings.llm_configuration.model_name,
                "llm_quantization": settings.llm_configuration.quantization.value,
                "llm_prompt": settings.llm_configuration.prompt,
            }
    ...
    def _backup_existing_file(self, export_path: FilePath,
        ...
        file_path = Path(export_path.value)
...
content_result = (
    self._serialization.serialize_to_json(data, request.configuration.pretty_format))
            elif request.export_format == ExportFormat.XML:
content_result = (
    self._serialization.serialize_to_xml(data, request.configuration.pretty_format))
            ...
            # Write to file
            file_path = Path(request.export_path.value,
    )
