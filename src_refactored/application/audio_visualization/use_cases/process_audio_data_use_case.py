...
class AudioDataValidationServiceProtocol(Protocol,
    ...
    def validate_raw_data(self, data: Any, data_type: AudioDataType,
    ) -> bool:
    ...
    def get_data_info(self, data: Any, data_type: AudioDataType,
    ) -> dict[str, Any]:
...
class AudioDataConversionServiceProtocol(Protocol):
    ...
    def convert_to_numpy(self, data: Any, data_type: AudioDataType,
    ) -> np.ndarray:
    ...
    def resample_data(self, data: np.ndarray, target_rate: int, current_rate: int,
    ) -> np.ndarray:
...
class AudioNormalizationServiceProtocol(Protocol):
    ...
    def normalize_for_speech(self, data: np.ndarray, scaling_factor: float = 0.3,
    ) -> np.ndarray:
    ...
    def normalize_rms_based(self, data: np.ndarray, target_rms: float = 0.1,
    ) -> np.ndarray:
    ...
    def normalize_peak_based(self, data: np.ndarray, target_peak: float = 1.0,
    ) -> np.ndarray:
...
if request.configuration.normalization_method = (
    = NormalizationMethod.SPEECH_OPTIMIZED:)
