...
class AudioNormalizationServiceProtocol(Protocol):
    ...
    def normalize_peak_based(
        ...
        Args:
            ...
            current_peak: Current peak value (calculated if not provided)
    ...
    def normalize_for_speech(
        ...
        Returns:
            Speech-optimized normalized audio data
        """
        ...

    def apply_z_score_normalization(self, data: np.ndarray) -> np.ndarray:
...
class AudioProcessingServiceProtocol(Protocol):
    ...
    def apply_scaling(self, data: np.ndarray, factor: float,
    ) -> np.ndarray:
