...
class WorkerRegistryProtocol(Protocol):
    ...
    def get_active_workers(self, worker_type: WorkerType | None = None) -> Result[list[Any]]:
    ...
    def get_worker_thread(self, worker: Any,
    ) -> Result[Any | None]:
        """Get thread associated with worker"""
        ...

    def remove_worker(self, worker: Any, worker_type: WorkerType,
    ) -> Result[None]:
        """Remove worker from registry"""
        ...

    def is_worker_active(self, worker: Any,
    ) -> bool:
...
class SignalManagementServiceProtocol(Protocol):
    ...
    def disconnect_specific_signals(self, worker: Any, signal_names: list[str]) -> Result[None]:
        """Disconnect specific signals from worker"""
        ...

    def get_connected_signals(self, worker: Any,
    ) -> Result[list[str]]:
        """Get list of connected signals for worker"""
        ...

    def verify_signals_disconnected(self, worker: Any,
    ) -> Result[bool]:
...
class ResourceManagementServiceProtocol(Protocol):
    ...
    def cleanup_worker_resources(self, worker: Any, worker_type: WorkerType,
    ) -> Result[None]:
...
class WorkerControlServiceProtocol(Protocol):
    ...
    def stop_worker(self, worker: Any, timeout_ms: int = 5000,
    ) -> Result[None]:
        """Stop worker gracefully"""
        ...

    def force_stop_worker(self, worker: Any, timeout_ms: int = 2000,
    ) -> Result[None]:
        """Force stop worker"""
        ...

    def is_worker_running(self, worker: Any,
    ) -> bool:
        """Check if worker is running"""
        ...

    def get_worker_status(self, worker: Any,
    ) -> Result[dict[str, Any]]:
        """Get worker status information"""
        ...

    def send_shutdown_signal(self, worker: Any,
    ) -> Result[None]:
 ...
 = (
    [status.worker_type for status in cleanup_result.worker_statuses if status.result != CleanupResult.SUCCESS])
            ...
            if not verification_result.is_success:
                response.warnings.append(f"Cleanup verification failed: {verification_result.error_m\
    essage}")
...
response.total_cleanup_time_ms = (
    ...
    ) -> list[tuple[Any, WorkerCleanupConfiguration]]:
...
worker_result = (
    ...
    def _cleanup_workers_by_strategy(
        self,
        workers_to_cleanup: list[tuple[Any, WorkerCleanupConfiguration]],
    ...
    80 // max(1, len(workers_to_cleanup))  # 80% for cleanup, ending at 90%)
        ...
        for i, (worker, config) in enumerate(workers_to_cleanup):
            ...
            if status.result != CleanupResult.SUCCESS:
                warnings.append(f"Worker {config.worker_type.value} cleanup failed: {status.error_me\
    ssage}")
...
wait_result = (
    self._thread_management.wait_for_thread(thread, config.graceful_timeout_ms))
                        if not wait_result.is_success:
                            self._logger.log_warning(f"Thread did not complete within timeout: {wait\
    _result.error_message}")

                    # Clean up thread resources
                    cleanup_result = self._thread_management.cleanup_thread(thread)
                    if not cleanup_result.is_success:
                        self._logger.log_warning(f"Thread cleanup failed: {cleanup_result.error_mess\
    age}")
                else:
                    self._logger.log_warning(f"Failed to stop thread: {thread_stop_result.error_mess\
    age}")
...
resource_result = (
    self._resource_management.cleanup_worker_resources(worker, config.worker_type))
                if resource_result.is_success:
                    ...
                    self._logger.log_warning(f"Resource cleanup failed: {resource_result.error_messa\
    ge}")
...
temp_cleanup_result = (
    self._resource_management.cleanup_temporary_files(config.worker_type))
                if not temp_cleanup_result.is_success:
                    self._logger.log_warning(f"Temporary file cleanup failed: {temp_cleanup_result.e\
    rror_message}")
...
gpu_cleanup_result = (
    self._resource_management.release_gpu_memory(config.worker_type))
                if not gpu_cleanup_result.is_success:
                    self._logger.log_debug(f"GPU memory cleanup not applicable or
    failed: {gpu_cleanup_result.error_message}")
            ...
            if not registry_result.is_success:
                self._logger.log_warning(f"Failed to remove worker from registry: {registry_result.e\
    rror_message}")
            ...
            # Determine success
            if status.signals_disconnected and
    status.worker_stopped and status.thread_stopped and status.resources_cleaned:
            ...
            status.cleanup_time_ms = int((datetime.utcnow() - start_time).total_seconds() * 1000,
    )
