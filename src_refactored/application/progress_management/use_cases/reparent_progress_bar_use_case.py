...
from src_refactored.domain.window_management.value_objects import ReparentDirection
...
class ReparentProgressBarRequest:
    ...
    configuration: ReparentConfiguration = None
...
progress_bar_errors = (
    self.widget_validation_service.validate_progress_bar(request.progress_bar))
...
target_errors = (
    self.widget_validation_service.validate_target_widget(request.target_widget))
            if target_errors:
                return self._create_error_response(
...
source_errors = (
    self.widget_validation_service.validate_target_widget(request.source_widget)
    )
                if source_errors:
                    return self._create_error_response(
...
source_state = (
    self.state_backup_service.backup_progress_bar_state(request.progress_bar,)
    )

                    # Backup additional properties if configured
                    if request.configuration.preserve_properties:
source_state.properties = (
    self.state_backup_service.backup_widget_properties()
                            request.progress_bar,
                        )
