...
class StartProgressSessionRequest:
    ...
    configuration: ProgressSessionConfiguration = None
    ...
    def __post_init__(self):
        ...
        if self.configuration is None:
            self.configuration = ProgressSessionConfiguration(
                disable_ui_elements=True,
                expand_progress_area=True,
                reparent_progress_bar=True,
            )
...
original_ui_state = (
    self.ui_state_service.store_original_ui_state(request.parent_window)
    )
            ...
            try:
                ...
                if progress_bar and request.configuration.reparent_progress_bar:
                    # Store original progress bar state
progress_bar_info = (
    self.progress_bar_service.store_progress_bar_state(progress_bar))

                    # Expand progress area if needed
                    if request.configuration.expand_progress_area and request.target_widget:
                        if self.ui_state_service.expand_progress_area(
...
progress_bar_info.current_location = (
    request.configuration.progress_bar_location)
