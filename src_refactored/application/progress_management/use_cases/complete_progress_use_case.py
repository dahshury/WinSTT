...
class CompleteProgressRequest:
    ...
    session_id: str
    configuration: CompletionConfiguration = None
    ...
    def __post_init__(self):
        ...
        if self.configuration is None:
            self.configuration = CompletionConfiguration()
...
class CompleteProgressUseCase:
    ...
    def execute(self, request: CompleteProgressRequest,
        ...
        try:
            ...
            if not request.force_completion and
    not self.session_lookup_service.is_session_active(request.session_id):
                return self._create_error_response(
...
validation_errors = (
    self.validation_service.validate_session_completion(request.session_id,)
    )
                ...
                try:
                    ...
                    if progress_bar_info and progress_bar_info.widget:
                        ...
                        if request.configuration.hide_progress_bar:
                            if not self.progress_bar_restoration_service.hide_progress_bar(progress_\
    bar):
                                restoration_state.restoration_warnings.append("Failed to hide progre\
    ss bar")

                        # Reset progress bar value
                        if not self.progress_bar_restoration_service.reset_progress_bar_value(progre\
    ss_bar):
                        ...
                        if progress_bar_info.original_parent:
                            if self.progress_bar_restoration_service.restore_progress_bar_parent(
                                ...
                                restoration_state.restoration_errors.append("Failed to restore progr\
    ess bar parent")

                        # Restore original geometry if available
                        if progress_bar_info.original_geometry:
                            if not self.progress_bar_restoration_service.restore_progress_bar_geomet\
    ry(
                                progress_bar,
                                progress_bar_info.original_geometry,
                            ):
                                restoration_state.restoration_warnings.append("Failed to restore pro\
    gress bar geometry")
...
successful_restorations = (
    sum(1 for success in restoration_results.values() if success))
                        ...
                        if successful_restorations == total_restorations:
                            ...
                            restoration_state.restoration_errors.append("Failed to restore any UI el\
    ements")
                ...
                if (request.configuration.collapse_progress_area and
                    ...
                    if target_widget:
                        if self.ui_restoration_service.collapse_progress_area(target_widget):
                            ...
                            restoration_state.restoration_warnings.append("Failed to collapse progre\
    ss area")
                ...
                if parent_window:
                    if self.layout_restoration_service.validate_layout_integrity(parent_window):
                        ...
                        restoration_state.restoration_warnings.append("Layout integrity validation f\
    ailed")
...
temp_widgets_cleaned = (
    self.cleanup_service.cleanup_temporary_widgets(request.session_id))
...
connections_cleaned = (
    self.cleanup_service.cleanup_event_connections(request.session_id))
            ...
            if request.configuration.validate_restoration:
validation_errors = (
    self.validation_service.validate_restoration_state(restoration_state))
                if validation_errors:
            ...
            if restoration_state.restoration_errors:
                result
 = (
    CompleteResult.UI_RESTORATION_ERROR if not restoration_state.progress_bar_restored else CompleteResult.PROGRESS_BAR_RESTORATION_ERROR)
            else:
