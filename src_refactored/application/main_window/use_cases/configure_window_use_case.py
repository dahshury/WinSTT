...
class ConfigureWindowRequest:
    ...
    property_updates: list[PropertyUpdate] = None
    geometry_updates: list[GeometryUpdate] = None
    style_updates: list[StyleUpdate] = None
...
window: Any, geometry_type: GeometryType, value: Any, animate: bool = (
    False, duration_ms: int = 250),
    ) -> bool:
...
class StyleManagementServiceProtocol(Protocol):
    ...
    def set_style(self,
window: Any, style_type: StyleType, value: Any, merge: bool = (
    True, apply_to_children: bool = False),
    ) -> bool:
...
errors = (
    self.validation_service.validate_property_update(request.window, prop_update))
                    validation_errors.extend(errors)

            for geom_update in request.geometry_updates:
                if geom_update.validate_constraints:
errors = (
    self.validation_service.validate_geometry_update(request.window, geom_update))
                    validation_errors.extend(errors)

            for style_update in request.style_updates:
                if style_update.validate_syntax:
errors = (
    self.validation_service.validate_style_update(request.window, style_update))
                    validation_errors.extend(errors)
...
backup = (
    self.backup_service.create_configuration_backup(request.window, backup_id))
...
error_msg = (
    f"Property update failed for {prop_update.property_type.value}: {e!s}")
                    property_failures.append(error_msg)
...
error_msg = (
    f"Geometry constraints validation failed for {geom_update.geometry_type.value}")
...
error_msg = (
    f"Geometry update failed for {geom_update.geometry_type.value}: {e!s}")
                    geometry_failures.append(error_msg)
                ...
                try:
                    ...
                    if style_update.validate_syntax:
                        ...
                        if syntax_errors:
                            error_msg
 = (
    f"Style syntax validation failed for {style_update.style_type.value}: {'; '.join(syntax_errors)}")
            ...
            try:
                ...
                for prop_update in request.property_updates:
current_properties[prop_update.property_type] = (
    self.property_service.get_current_property()
                        request.window,
                        prop_update.property_type,
                    )
                ...
                for geom_update in request.geometry_updates:
current_geometry[geom_update.geometry_type] = (
    self.geometry_service.get_current_geometry()
                        request.window,
                        geom_update.geometry_type,
                    )
...
total_updates = (
    len(request.property_updates) + len(request.geometry_updates) + len(request.style_updates))
