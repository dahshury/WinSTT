...
if request.translation_config and request.translation_config.mode ! = (
        ...
        try:
            ...
            for text_content in request.text_updates:
                ...
                try:
                    ...
                    state.errors.append(f"Invalid encoding {text_content.encoding} for text ID: {tex\
    t_content.text_id}")
                    return False

                # Validate length
                if text_content.max_length and len(text_content.content) > text_content.max_length:
                    state.errors.append(f"Text too long for ID {text_content.text_id}: {len(text_con\
    tent.content)}
            ...
            for target in request.target_widgets:
                if not self._widget_text.validate_widget_text_property(target.widget_id, target.text_property):
                    state.errors.append(f"Invalid text property {target.text_property} for widget {t\
    arget.widget_id}")
                    return False

            # Validate translation configuration
            if request.translation_config:
                if not request.translation_config.source_language or
    not request.translation_config.target_language:
                    state.errors.append("Translation configuration missing source or
    target language")
                    return False
        ...
        try:
            for text_content in request.text_updates:
                ...
                if request.validation_config:
                    ...
                    if not is_valid:
                        state.warnings.append(f"Text content validation failed for ID: {text_content\
    .text_id}")
        ...
        for text_content in request.text_updates:
            try:
                ...
                if cached_translation:
                    ...
                    if translation_result.translation_successful and
    request.translation_config.use_cache:
...
self._logger.log_error(f"Translation failed for text ID {text_content.text_id}", error = (
    e))
...
self._logger.log_error(f"Formatting failed for text ID {text_content.text_id}", error = (
    e))
    ...
    def _update_widget_text(self, request: UpdateUITextRequest, state: UITextUpdateState,
        ...
        for target in sorted_targets:
            ...
            matching_text = None
            for text_content in request.text_updates:
                if text_content.text_id in target.widget_id or
    target.widget_id in text_content.text_id:
            ...
            try:
                # Get current text
current_text = (
    self._widget_text.get_widget_text(target.widget_id, target.text_property))
...
self._logger.log_error(f"Failed to update text for widget {target.widget_id}", error = (
    e))
    ...
    request: UpdateUITextRequest, state: UITextUpdateState, text_updates: list[TextUpdate]) -> bool:
