...
from src_refactored.domain.media.value_objects.conversion_operations import (
    AudioFormat,
    ConversionPhase,
    ConversionResult,
    ConversionStrategy,
...
class ConvertVideoUseCase:
    ...
    def execute(self, request: ConvertVideoRequest,
        ...
        try:
            ...
            # Initialize progress tracking
total_steps = (
    6  # validation, preparation, extraction, processing, finalization, completion)
...
is_valid, validation_error = (
    self._video_validation_service.validate_video(request.video_path))
            if not is_valid:
                return self._create_error_response(
                    ConversionResult.INVALID_INPUT,
            ...
            if request.output_path:
                ...
                output_base_path = os.path.splitext(request.video_path,
    )[0]
