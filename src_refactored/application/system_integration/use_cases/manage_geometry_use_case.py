...
class AnimationConfiguration:
    ...
    callback_on_finished: callable | None = None
...
class ConstraintCheckResult:
    ...
    constraint_messages: list[str] = None
...
class ManageGeometryResponse:
    ...
    phase_times: dict[ManagePhase, float] = None
...
class ManageGeometryUseCase:
    ...
    def __init__(
        ...
        geometry_application_service: GeometryApplicationServiceProtocol = None,
...
widget_valid, widget_error = (
    self._validation_service.validate_widget(request.target_widget))
            if not widget_valid:
            ...
            if request.position_config:
                position_valid,
position_error = (
    self._validation_service.validate_position_configuration(request.position_config))
...
size_valid, size_error = (
    self._validation_service.validate_size_configuration(request.size_config))
                if not size_valid:
...
constraints_valid, constraint_errors = (
    self._validation_service.validate_constraints(request.constraints))
...
target_screen_index = (
    request.position_config.screen_index if request.position_config else None)
target_screen = (
    self._screen_analysis_service.get_target_screen(request.target_widget, target_screen_index))
...
current_geometry = (
    self._geometry_application_service.get_current_geometry(request.target_widget))
            ...
            if request.animation_config and request.animation_config.animation_type
             !=  AnimationType.NONE and self._animation_service:
                animation_created,
animation_object, animation_error = (
    self._animation_service.create_geometry_animation()
                    request.target_widget,
                    geometry_calculation.calculated_geometry,
            ...
            use_animation
 = (
    animation_setup.animation_ready and request.animation_config and request.animation_config.auto_start)

            if use_animation and self._animation_service:
                # Start animation
                animation_started,
animation_error = (
    self._animation_service.start_animation(animation_setup.animation_object))
            ...
            if not use_animation:
                ...
                geometry_applied,
final_geometry, application_error = (
    self._geometry_application_service.apply_geometry()
                    request.target_widget,
                    geometry_calculation.calculated_geometry,
                    animated=False,
                )
            ...
            geometry_application = GeometryApplicationResult(
                ...
                animation_started=use_animation,
            ...
            if request.persistence_config and
    request.persistence_config.auto_save and self._state_persistence_service:
state_saved, persistence_error = (
    self._state_persistence_service.save_geometry_state()
                    request.target_widget,
                    final_geometry,
...
state_persistence.position_saved = (
    state_saved and request.persistence_config.save_position)
            ...
            if request.enable_logging and self._logger_service:
                self._logger_service.log_info(
                    ...
                    operation=request.operation.value,
final_geometry = (
    f"{final_geometry.x},{final_geometry.y} {final_geometry.width}x{final_geometry.height}",)
...
animation_controller = (
    animation_setup.animation_object if animation_setup.animation_ready else None,)
                warnings=warnings,
                execution_time=time.time() - start_time,
                phase_times=phase_times,
            )
