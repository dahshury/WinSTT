...
class CoordinationConfiguration:
    ...
    thread_dependencies: dict[str, list[str]] = None
    synchronization_points: list[str] = None
    communication_channels: dict[str, str] = None
...
class WorkerThreadsState:
    ...
    thread_results: list[ThreadSetupResult] = None
...
class SetupWorkerThreadsResponse:
    ...
    phase_times: dict[SetupPhase, float] = None
...
resources_available, resource_error = (
    self._thread_validation_service.check_resource_availability())
...
coord_valid, coord_error = (
    self._thread_validation_service.validate_coordination_configuration()
                request.coordination_config,
            )
            if not coord_valid:
...
priority_set, priority_error = (
    self._thread_creation_service.configure_thread_priority()
                        thread_obj, config.priority,
                    )
                    if not priority_set and priority_error:
                        warnings.append(f"Failed to set priority for {config.thread_id}: {priority_e\
    rror}")
...
deps_established, deps_count, deps_error = (
    self._coordination_service.establish_dependencies()
                request.coordination_config.thread_dependencies, thread_map,
            )
...
sync_created, sync_count, sync_error = (
    self._coordination_service.create_synchronization_points()
                request.coordination_config.synchronization_points,
            )
            if not sync_created:
...
channels_setup, channels_count, channels_error = (
    self._coordination_service.setup_communication_channels()
                request.coordination_config.communication_channels,
            )
            ...
            lifecycle_setup,
lifecycle_manager, lifecycle_error = (
    self._lifecycle_management_service.setup_lifecycle_management()
...
shutdown_installed, shutdown_error = (
    self._lifecycle_management_service.install_shutdown_handlers()
                created_threads,
            )
...
cleanup_installed, cleanup_error = (
    self._lifecycle_management_service.install_cleanup_handlers()
                created_threads,
            )
...
exception_setup, exception_error = (
    self._lifecycle_management_service.setup_exception_handling()
                created_threads,
            )
