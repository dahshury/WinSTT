...
class InitializeSystemTrayResponse:
    ...
    phase_times: dict[InitializePhase, float] = field(default_factory=dict)
...
class InitializeSystemTrayUseCase:
    ...
    def execute(self, request: InitializeSystemTrayRequest,
        ...
        try:
            ...
            icon_valid, icon_error = self._tray_validation_service.validate_icon_configuration(request.icon_config)
            if not icon_valid:
            ...
            # Validate menu configuration
            menu_valid, menu_error = self._tray_validation_service.validate_menu_configuration(request.menu_config)
            if not menu_valid:
            ...
            event_valid, event_error = self._tray_validation_service.validate_event_configuration(request.event_config)
            if not event_valid:
            ...
            icon_loaded, icon_widget, icon_error = self._icon_management_service.load_icon(request.icon_config)
            if not icon_loaded:
            ...
            # Bind menu actions
            actions_bound, actions_count, actions_error = self._menu_creation_service.bind_menu_actions(
                menu_widget, request.menu_config.menu_items,
            )
            ...
            state.menu_setup = MenuSetup(
                ...
                separators_count=sum(1 for item in request.menu_config.menu_items if item.item_type == MenuItemType.SEPARATOR),
                submenus_count=sum(1 for item in request.menu_config.menu_items if item.item_type == MenuItemType.SUBMENU),
            ...
            # Set tray icon
            icon_set, icon_set_error = self._tray_creation_service.set_tray_icon(tray_widget, icon_widget)
            if not icon_set:
                warnings.append(f"Failed to set tray icon: {icon_set_error}")

            # Set tray menu
            menu_set, menu_set_error = self._tray_creation_service.set_tray_menu(tray_widget, menu_widget)
            if not menu_set:
            ...
            # Bind click events
            clicks_bound, clicks_count, clicks_error = self._event_binding_service.bind_click_events(
            ...
            # Setup notifications
            notifications_setup, notifications_error = self._event_binding_service.setup_notifications(
