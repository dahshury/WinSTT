...
from ....domain.transcription.value_objects import TranscriptionResult, TranscriptionState
from ....domain.transcription.value_objects.transcription_operations import (
    FilterCriteria,
    SortOrder,
)
...
class GetTranscriptionHistoryRequest:
    ...
    filter_criteria: list[FilterCriteria] = None
...
session_history = (
    self._transcription_session.get_session_history(request.session_id))
            else:
    ...
    def _convert_to_history_items(
        ...
        for result in history:
            ...
            if hasattr(result, "model_configuration") and result.model_configuration:
                model_type = str(result.model_configuration.model_type)
                model_size = str(result.model_configuration.model_size,
    )
...
model_name = (
    f"{item.model_configuration.model_type}_{item.model_configuration.model_size}")
                models.append(model_name)

        most_used_model = max(set(models,
    ), key=models.count) if models else None

        # Calculate success rate
success_rate = (
    completed_transcriptions / total_transcriptions if total_transcriptions > 0 else 0.0)
