    ...
    def execute(self, request: SaveConfigurationRequest,
        ...
        try:
            ...
            if estimated_size_result.is_success:
                ...
                if available_space_result.is_success and available_space_result.value < estimated_size * 2:
            ...
            if request.save_config.create_backup and self._file_system.file_exists(request.save_config.target_path):
                ...
                if backup_result.is_success:
                    ...
                    response.validation_warnings.append(f"Failed to create backup: {backup_result.error_message}")
            ...
            if target_dir:
                ...
                if not dir_result.is_success:
                    response.error_message = f"Failed to create target directory: {dir_result.error_message}"
                    return response
            ...
            if not write_result.is_success:
                ...
                if backup_path and request.save_config.strategy == SaveStrategy.ATOMIC:
                    ...
                    if not restore_result.is_success:
                        response.validation_warnings.append(f"Failed to restore backup: {restore_result.error_message}")
                return response
            ...
            if request.save_config.verify_after_save:
                ...
                if not verify_result.is_success:
                    response.validation_warnings.append(f"Save verification failed: {verify_result.error_message}")
            ...
            if not history_result.is_success:
                response.validation_warnings.append(f"Failed to update save history: {history_result.error_message}")
