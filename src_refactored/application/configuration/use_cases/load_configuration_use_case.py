...
class FileSystemServiceProtocol(Protocol):
    ...
    def get_project_root(self) -> Result[Path]:
    ...
    int((datetime.utcnow() - start_time).total_seconds() * 1000))
            ...
            if not config_data_result.is_success:
                if request.configuration.fallback_to_defaults:
                    self._logger.log_warning(f"Failed to load configuration,
                    using defaults: {config_data_result.error_message}")
...
response.error_message = (
    f"Failed to merge with defaults: {final_config_result.error_message}")
                return response
...
response.error_message = (
    f"Failed to create user preferences: {prefs_result.error_message}")
                return response
            ...
            if request.configuration.cache_loaded_config:
                ...
                if not cache_result.is_success:
                    response.validation_warnings.append(f"Failed to cache configuration: {cache_resu\
    lt.error_message}")
            ...
            try:
                ...
                if config_result.is_success:
                    ...
                    if merge_result.is_success:
                        ...
                        self._logger.log_warning(f"Failed to merge config from {location.source}: {m\
    erge_result.error_message}")
                else:
                    self._logger.log_warning(f"Failed to load config from {location.source}: {config\
