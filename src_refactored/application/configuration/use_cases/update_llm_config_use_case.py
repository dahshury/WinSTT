...
new_quantization = (
    request.update.quantization_level.value if request.update.quantization_level else None,),
            ...
            enabled_to_update
 = (
    request.update.enabled if request.update.enabled is not None else request.current_enabled)
...
quantization_to_update = (
    request.update.quantization_level or request.current_quantization)
            ...
            if enabled_to_update and
    (not model_to_update or not quantization_to_update or not prompt_to_update):
response.error_message = (
    "Model, quantization, and prompt must be specified when LLM is enabled")
                return response
...
quantization_validation = (
    self._llm_validation.validate_llm_quantization_level(quantization_to_update,)
    )
                if not quantization_validation.is_success:
response.error_message = (
    f"Invalid LLM quantization: {quantization_validation.error_message}")
 ...
 = (
    f"LLM model {model_to_update.value} is incompatible with quantization {quantization_to_update.value}")
                    ...
                    if compatibility_result.value == LLMCompatibility.PARTIALLY_COMPATIBLE:
                        response.warnings.append(f"LLM model {model_to_update.value} has limited com\
    patibility with quantization {quantization_to_update.value}")
...
response.error_message = (
    f"Invalid LLM prompt: {prompt_validation.error_message}")
...
response.error_message = (
    f"Failed to update LLM configuration: {config_update_result.error_message}")
...
response.error_message = (
    f"Failed to restart LLM worker: {worker_start_result.error_message}")
        ...
        try:
            ...
            if self._llm_worker_management.is_llm_worker_running():
                ...
                if not disable_result.is_success:
                    response.warnings.append(f"Failed to disable LLM worker: {disable_result.error_m\
    essage}")
            ...
            if not config_update_result.is_success:
                response.error_message
                 =  f"Failed to disable LLM in configuration: {config_update_result.error_message}"
                return response
            ...
            if request.update.save_to_persistent_config:
                ...
                if save_result.is_success:
                    ...
                    response.warnings.append(f"Failed to save configuration: {save_result.error_mess\
    age}")
