...
class ColorPalette(ValueObject[dict[ColorRole, Color]]):
    """Color palette value object.
    ...
    @classmethod
    def from_dict(cls,
color_dict: dict[str, str | tuple[int, int, int] | tuple[int, int, int, int]], theme: PaletteTheme = (
    PaletteTheme.CUSTOM,),
    ) -> Result[ColorPalette]:
            ...
            for role_name, color_value in color_dict.items():
                ...
                if isinstance(color_value, str):
                    ...
                    if len(color_value) == 3 or len(color_value) == 4:
                        ...
                        return Result.failure(f"Invalid color tuple length for {role_name}: {len(col\
    or_value)}")
                else:
    ...
    def get_color_or_default(self, role: ColorRole, default: Color,
        ...
        """
        return self.value.get(role, default)
    ...
    def merge_with(self, other: ColorPalette,
        ...
        """
        merged_colors = self.value.copy()
        merged_colors.update(other.value)
    ...
    def to_dict(self, hex_format: bool = True,
        ...
        """
        result = {}
        for role, color in self.value.items():
            if hex_format:
    ...
    def validate_completeness(self, required_roles: set[ColorRole] | None = None) -> Result[None]:
        ...
        missing_roles = required_roles - set(self.value.keys(),
    )
        if missing_roles:
    ...
    def __repr__(self) -> str:
        ...
        return f"ColorPalette(theme={self._theme.value}, colors={dict(self.value)})"