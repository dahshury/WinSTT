...
class ZOrderLevel(ValueObject[int]):
    """Z-order level value object.
    ...
    def __init__(self, value: int,
        ...
        if not isinstance(value, int):
            try:
                ...
                msg = f"Z-order value must be an integer, got {type(value)}"
                raise ValueError(msg)

        if not (self.MIN_Z_ORDER <= value <= self.MAX_Z_ORDER):
            msg = (
