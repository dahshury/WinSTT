"""Audio Normalization Service for speech-specific audio processing.

This module implements the AudioNormalizationService that provides
speech-optimized audio normalization and processing.
Extracted from src/ui/voice_visualizer.py lines 140-154.
"""
...
from src_refactored.domain.audio_visualization.value_objects.normalization_configuration import (
    NormalizationConfig,
)
...
class AudioNormalizationServiceProtocol(Protocol):
    ...
    def normalize_for_speech(self, audio_data: np.ndarray, config: NormalizationConfig,
    ) -> np.ndarray:
        """Normalize audio data for speech visualization."""
        ...
    ...
    def apply_dynamic_range_compression(self, audio_data: np.ndarray, ratio: float,
    ) -> np.ndarray:
...
class SpeechNormalizer:
    """Speech-optimized audio normalizer.
    
    Implements speech-specific normalization algorithms optimized
    for voice visualization and transcription applications.
    """
    ...
    def __init__(self, config: NormalizationConfig,
        ...
        """
        self.config = config
        self.logger = LoggerService().get_logger("SpeechNormalizer")
        self._last_rms = 0.0
...
class NormalizationProcessor(QObject):
    ...
    def __init__(self, config: NormalizationConfig, parent: QObject | None = None):
        ...
        self.logger = LoggerService().get_logger("NormalizationProcessor")
    ...
    def update_config(self, new_config: NormalizationConfig,
    ) -> None:
...
class AudioNormalizationService:
    ...
    self,
    audio_data: np.ndarray,
    config: NormalizationConfig | None = None) -> np.ndarray:
        """Normalize audio data for speech visualization.
        ...
        Returns:
            Normalized audio data
        """
        if config and config != self.processor.config:
            self.processor.update_config(config)

        normalizer = SpeechNormalizer(config or self.default_config)
        return normalizer.normalize_for_speech(audio_data)
    ...
    def calculate_rms(self, audio_data: np.ndarray) -> float:
        ...
        return normalizer.calculate_rms(audio_data)
    ...
    def create_processor(self, config: NormalizationConfig | None = None) -> NormalizationProcessor:
        ...
        self.logger.info("Created normalization processor with method: {processor_config.method.valu\
    e}")
        return processor
    ...
    sensitivity: float = 0.1) -> NormalizationConfig:
