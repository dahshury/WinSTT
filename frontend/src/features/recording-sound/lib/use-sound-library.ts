...
import { dialogOpenFile, soundLibraryAdd, soundLibraryRemove } from "@/shared/api/ipc-client";
...
interface UseSoundLibraryReturn {
	...
	addFromPath: (sourcePath: string, displayName?: string) => Promise<SoundLibraryItem | null>;
...
const SOUND_FILE_FILTERS = [{ name: "Audio", extensions: ["wav", "mp3"] }];
 ...
 */
function deriveLibrary(general: GeneralSlice, defaultName: string): DerivedLibrary {
	...
	const activeItem = items.find((it) => isActive(it, activePath)) ?? defaultEntry;
 ...
 */
function addedEntry(result: { ok: boolean; entry?: SoundLibraryEntry }): SoundLibraryEntry | null {
...
function removeErrorMessage(result: { ok: boolean; error?: string }): string | null {
	if (result.ok) {
		return null;
	}
...
export function useSoundLibrary({
	...
	const { activePath, library, defaultEntry, items, activeItem } = deriveLibrary(
		general,
		defaultName
	);
	...
	const addFromPath = useCallback(
		async (sourcePath: string, displayName?: string): Promise<SoundLibraryItem | null> => {
			const outcome = readAddResult(await soundLibraryAdd(sourcePath, displayName));
			if ("error" in outcome) {
	...
	const addFromBrowse = useCallback(async (): Promise<SoundLibraryItem | null> => {
		const sourcePath = await dialogOpenFile(SOUND_FILE_FILTERS, "Select Recording Sound");
		if (!sourcePath) {
			return null;
		}
		return addFromPath(sourcePath);
	}, [addFromPath]);
