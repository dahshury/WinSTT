...
import { isRealtimeViable } from "./realtime-viability";
...
/** Distinct explicit language codes across the catalog, sorted. */
export function collectFilterableLanguages(models: readonly ModelInfo[]): string[] {
...
function modelMatchesSearch(m: ModelInfo, query: string): boolean {
	...
	const haystack = `${m.displayName} ${m.id} ${m.family} ${m.sizeLabel}`.toLowerCase();
...
function passesCachedFilter(filters: SttFilterState, entry: ModelStateEntry | undefined): boolean {
	return !filters.cachedOnly || entry?.cache.state === "cached";
}

function passesRealtimeFilter(filters: SttFilterState, m: ModelInfo): boolean {
	return !filters.realtimeOnly || isRealtimeViable(m);
}
...
function matchesAllFilters(m: ModelInfo, query: string, ctx: FilterModelsContext): boolean {
	const entry = ctx.statesById[m.id];
	const checks = [
		modelMatchesSearch(m, query),
		passesCachedFilter(ctx.filters, entry),
		passesRealtimeFilter(ctx.filters, m),
