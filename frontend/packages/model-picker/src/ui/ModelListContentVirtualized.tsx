...
import {
	...
	getVariantClasses,
	getVariantIcon,
} from "../lib/model-selector-display-utils";
...
function ProvidersRowInner({
	...
	// Shared <Collapsible> owns the grid-rows animation, lazy mount, and
	// inert+data-state contract. The wrapping ``Collapsible`` already
	// gates mount via :func:`useOpenedFlag`, so the inner grid is only
	// constructed after the first open.
	return (
...
/**
 * Back-compat alias for the older test surface (`useProvidersOpenedFlag`).
 * Delegates to the shared :func:`useOpenedFlag` so there's exactly one
 * implementation of the "first-open latch" pattern across pickers.
 */
...
function MakerIcon({ maker }: { maker: string | undefined }) {
	...
	return (
		<span className="flex size-4 shrink-0 items-center justify-center overflow-hidden rounded border border-border/50 bg-surface p-0.5">
			{/** biome-ignore lint/performance/noImgElement: Provider icons are static local PNG/SVGs served from /public; next/image adds runtime overhead for tiny 16x16 thumbnails. */}
			<img
...
export interface ModelListContentVirtualizedProps {
	...
	onActiveMakerChange?: (maker: string | null) => void;
	onSelectModel: (modelId: string | undefined, providerSlug?: string) => void;
...
function getRowKey(item: VirtualizedItem): string {
...
export const ModelListContentVirtualized = memo(function ModelListContentVirtualized({
	...
	scrollToMakerRequest,
	onActiveMakerChange,
}: ModelListContentVirtualizedProps) {
...
function EmptyState({ hasActiveFilters }: { hasActiveFilters: boolean }): ReactNode {
	return (
