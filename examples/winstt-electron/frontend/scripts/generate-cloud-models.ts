 ...
 * unquoted, so it's naturally skipped) and emit a sibling `*.generated.ts`.
 *
 ...
 *
 * Re-run via `bun generate` (chained after the spec/zod generation) after
 * bumping the `@ai-sdk` packages to pick up newly-supported models. The output
 * is committed (like `schema.zod.ts`) so builds don't depend on regen.
 *
 * Standalone: `bun run scripts/generate-cloud-models.ts`
 */
...
function renderFile(ids: Record<"openai" | "elevenlabs", string[]>): string {
	...
	return `// AUTO-GENERATED by scripts/generate-cloud-models.ts — DO NOT EDIT.
//
