...
function Header({ ctl, now }: { ctl: ContextPlaygroundController; now: number }) {
		...
		<header className="flex shrink-0 flex-wrap items-center gap-2 border-border border-b bg-surface-primary px-3 py-2">
			...
			<button
				className={cn(
					...
					live ? "bg-success-dim text-success" : "bg-surface-tertiary text-foreground-muted"
				)}
	...
	return (
		<>
			{waiting === "own-window-focused" && (
				<Banner tone="muted">
					Showing the last external capture. Click into a field in another app to refresh.
				</Banner>
			)}
			{!report.contextAwarenessEnabled && (
				<Banner tone="warning">
					Context awareness is <b>OFF</b> in settings; dictation won't use any of this. The
					playground still shows what <i>would</i> be captured.
				</Banner>
			)}
			{report.denied && (
				<Banner tone="error">
					Deny-listed{report.deniedReason ? ` by "${report.deniedReason}"` : ""}: sensitive fields
					stripped before the model sees them.
				</Banner>
			)}

			<Summary report={report} />
			<Section subtitle={`${report.metrics.promptFragmentChars} chars`} title="LLM prompt fragment">
				...
				{report.asrPromptTailRaw !== report.asrPromptTail && (
					<div className="mt-2">
						<div className="mb-0.5 text-[10px] text-foreground-dim">
							raw textBefore — before sanitize + 250-char cap ({report.asrPromptTailRaw.length}{" "}
							chars)
						</div>
...
function RawJsonBlock({ report }: { report: ContextDebugReport }) {
	...
	return (
		<Section subtitle={`${json.length} chars · click to select all, then Ctrl+C`} title="Raw JSON">
			<textarea
...
function EmptyHint({ waiting }: { waiting: ContextPlaygroundController["waiting"] }) {
	return (
		<div className="flex h-full flex-col items-center justify-center gap-3 text-center text-foreground-muted">
			...
			<p className="max-w-sm text-body-sm">
				Click into any text field in another app (email body, browser input, your editor) to see
				exactly what WinSTT's context awareness captures from it.
			</p>
			{waiting === "live-off" && (
				<p className="text-[11px] text-foreground-dim">Live polling is paused.</p>
			)}
		</div>
	);
}
...
function Summary({ report }: { report: ContextDebugReport }) {
	...
	return (
		<div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
			...
			/>
			<Chip className="col-span-2 sm:col-span-3" label="Window" value={s.windowTitle || "—"} />
			<Chip
...
function Chip({
	...
	return (
		...
		>
			<span className="text-[10px] text-foreground-dim uppercase tracking-wide">{label}</span>
			<span className={cn("truncate font-mono text-[11px]", toneClass(tone))} title={value}>
				{value}
			</span>
		</div>
	);
	...
	return (
		<div className="rounded border border-border bg-surface-secondary p-2">
			<div className="mb-1 flex items-center gap-2">
				<span className="font-mono font-semibold text-[11px]">{result.mode}</span>
				<span
					className={cn(
						"rounded px-1.5 py-0.5 text-[10px]",
						result.ok ? "bg-success-dim text-success" : "bg-surface-tertiary text-foreground-dim"
					)}
				>
					{result.ok ? "content" : "empty"}
				</span>
				<span className="ml-auto text-[10px] text-foreground-muted">{result.durationMs}ms</span>
			</div>
	...
	return (
		<section className="rounded-md border border-border bg-surface-primary p-2">
			<div className="mb-1.5 flex items-baseline gap-2">
				...
				{subtitle && <span className="text-[10px] text-foreground-dim">{subtitle}</span>}
			</div>
			{children}
		</section>
	);
...
function Pre({ value, tall }: { tall?: boolean; value: string | undefined }) {
	...
	return (
		...
		>
			{text.length > 0 ? text : <span className="text-foreground-dim">(empty)</span>}
		</pre>
	);
}
...
function Banner({
	...
	return (
		<div className={cn("rounded border px-2.5 py-1.5 text-[11px]", bannerToneClass(tone))}>
			{children}
		</div>
	);
}
