...
function OptionContent({ active, option }: { active?: boolean; option: SelectOption }) {
	return (
		<>
...
function SelectGroupHeader({ badge, label }: { badge?: string | undefined; label: string }) {
	return (
								...
								{groups
									? groups.map((group) => (
											<Menu.Group className="flex flex-col" key={group.value}>
												<SelectGroupHeader badge={group.badge} label={group.label} />
												{group.options.map((opt) => (
													<SelectRow key={opt.id} option={opt} value={value} />
												))}
											</Menu.Group>
										))
									: flat.map((opt) => <SelectRow key={opt.id} option={opt} value={value} />)}
							</Menu.RadioGroup>
						</Menu.Popup>
					</Menu.Positioner>
				</SurfaceProvider>
