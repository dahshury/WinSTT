...
class ModelDownloadService(QObject):
    ...
    # Global lock to serialize all model downloads across services
    ...
    def __init__(self, config: ModelDownloadConfig,
        ...
        # Default to src/cache when not absolute
        base_cache = Path(config.cache_path)
        if not base_cache.is_absolute():
            # Resolve relative to src root
            base_cache = Path(__file__).resolve().parents[2] / base_cache
        self.cache_path = base_cache
    ...
    def download_whisper_models(self) -> bool:
        ...
        """
        try:
            with ModelDownloadService._global_download_lock:
            ...
            # Download ONNX model files (serialized)
            ...
            # Download configuration files under model_dir (serialized)
    ...
    def _download_file_with_progress(self, url: str, save_path: Path, filename: str) -> bool:
        ...
        try:
            ...
            # Stream to temp file with throttled progress emissions to avoid UI flicker
            ...
            with open(temp_path, file_mode) as file:
                for data in response.iter_content(self.config.chunk_size):
                    ...
                    now = time.time()
                    if percentage != last_emit_percent and (now - last_emit_time) >= emit_interval_s:
    ...
    def _validate_downloaded_file(self, temp_path: Path, final_path: Path,
        ...
        try:
            ...
            # ONNX file validation
                ...
                # Size heuristic: real Whisper ONNX files are large (> 1 MB)
                if file_size < 1_000_000:
                    return False
                # HTML check regardless of size
                try:
                    with open(temp_path, "rb") as f:
                        content_peek = f.read(2048).lower()
                        if b"<html" in content_peek or b"<!doctype" in content_peek:
                            self.logger.error("Downloaded file contains HTML, not ONNX: %s", final_path)
                            return False
                except Exception:
                    # If we cannot read the file as text at all, allow; size check still applied
                    pass
