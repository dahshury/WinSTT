...
from src.infrastructure.system.logging_service import LoggingService
    ...
    def initialize_audio(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Downgrade noisy message when no input device is present
                msg = str(e)
                if (
                    "Invalid input device" in msg
                    or "no default output device" in msg
                    or "Errno -9996" in msg
                ):
                    self.logger.debug("Float32 format open failed (no device). Trying fallback.")
                else:
            ...
            message = str(e)
            if (
                "Invalid input device" in message
                or "no default output device" in message
                or "Errno -9996" in message
            ):
                # Emit a single friendly message; avoid multiple warnings in console
                self.error_occurred.emit("No recording device detected. Please connect a microphone.")
                self.logger.debug("Audio init failed due to no input device: %s", message)
                return False
            self.logger.error("Failed to initialize audio: %s", e)
...
class AudioProcessorService:
    ...
    def create_processor(self, config: AudioProcessorConfig,
        ...
        from src.domain.common.ports.concurrency_management_port import (
        ...
        # Ensure base Entity lifecycle fields exist for dataclass-based entity
        try:
            if not hasattr(self.processor, "_created_at"):
                ts = DomainIdentityGenerator.generate_timestamp()
                self.processor._created_at = ts
                self.processor._updated_at = ts
        except Exception:
            pass
    ...
    def stop_processor(self, processor: AudioProcessorEntity,
        ...
        try:
            # Signal thread to stop and wait for it to finish to ensure
            # cleanup happens on the owning thread (inside run() finally)
            pyaudio_processor.stop()
            # Join QThread to avoid race conditions with resource cleanup
            try:
                pyaudio_processor.wait()
            except Exception:
                pass
    ...
    def cleanup_processor(self, processor: AudioProcessorEntity,
        ...
        if pyaudio_processor:
            try:
                # Ensure the worker thread has fully exited before cleanup
                if pyaudio_processor.isRunning():
                    try:
                        pyaudio_processor.stop()
                    except Exception:
                        pass
    ...
    def cleanup_all(self) -> None:
        ...
        for processor_id, pyaudio_processor in list(self._active_processors.items()):
            try:
                # Stop and join thread; resource cleanup will be performed
                # on the worker thread in its run() finally block
