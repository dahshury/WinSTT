...
from src.domain.audio.value_objects.audio_format import (
    AudioFormat,
    AudioFormatType,
    BitDepth,
)
from src.domain.audio.value_objects.listener_operations import (
    ListenerEvent,
    ListenerEventData,
    ListenerState,
)
...
class ConsolidatedListenerConfiguration:
    ...
    start_sound_file: str = "@resources/splash.wav"
...
class ConsolidatedListenerService:
        ...
        try:
            ...
            class _HotkeyAdapter:
                def on_hotkey_pressed(self, combo):
                    # Offload heavy work to a background thread to avoid running inside low-level hook callback
                    def _do_start():
                        outer_self._is_hotkey_pressed = True
                        outer_self._emit_event(ListenerEvent.HOTKEY_PRESSED)
                        outer_self._start_recording_internal()
                    threading.Thread(target=_do_start, daemon=True).start()

                def on_hotkey_released(self, combo):
                    # Offload heavy work to a background thread to avoid running inside low-level hook callback
                    def _do_stop():
                        outer_self._is_hotkey_pressed = False
                        outer_self._emit_event(ListenerEvent.HOTKEY_RELEASED)
                        outer_self._stop_recording_internal()
        ...
        try:
            ...
            if self._current_hotkey and self._keyboard_service.is_combination_pressed(self._current_hotkey):
                if not self._is_hotkey_pressed:
                    # Offload start to background thread
                    def _do_start():
                ...
                # Offload stop to background thread
                def _do_stop():
                    self._is_hotkey_pressed = False
    ...
    def _start_recording_internal(self) -> bool:
        ...
        try:
            ...
            # Start recording
            from src.domain.audio.value_objects.audio_configuration import (
                RecordingConfiguration,
            )
            from src.domain.audio.value_objects.recording_operation import (
                RecordingOperation,
            )
            from src.domain.audio.value_objects.service_requests import (
    ...
    def _process_recorded_audio(self) -> None:
        ...
        try:
            ...
            # Check minimum duration; suppress when no input device is available
            if self._current_session.duration < self._config.min_recording_duration:
                try:
                    from src.infrastructure.audio.pyaudio_service import pyaudio as _pa  # type: ignore
                except Exception:
                    _pa = None
                no_device = False
                if _pa is not None:
    ...
    def _save_audio_data(self, audio_data: AudioData,
        ...
        try:
            ...
            from src.domain.audio.entities.audio_file import FilePath
