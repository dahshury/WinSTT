...
class TrayIconService(QObject):
    ...
    def create_tray_icon(self) -> bool:
        ...
        # Connect signals
        if self.tray_icon is not None:
            # Connect directly; inside handler we defensively normalize the enum
    ...
    def cleanup(self) -> None:
        ...
        try:
            if self.tray_icon is not None:
                # Disconnect to prevent late signal deliveries during interpreter shutdown
                with suppress(Exception):
    ...
    def _on_tray_activated(self, reason) -> None:
        ...
        # If the app is closing, ignore late events
        app = QApplication.instance()
        if app is not None:
            with suppress(Exception):
                if app.closingDown():
        ...
        # Normalize to int for our public signal, but compare using enum where possible
        try:
            numeric_reason = reason.value if hasattr(reason, "value") else int(reason)
        except Exception:
            numeric_reason = -1

        with suppress(Exception):
            # Emit as plain int to avoid enum conversion issues
            self.tray_activated.emit(int(numeric_reason))

        # Ensure right-click shows context menu reliably
        try:
            activation_reason = reason if isinstance(reason, QSystemTrayIcon.ActivationReason) else QSystemTrayIcon.ActivationReason(numeric_reason)
        except Exception:
    ...
    def _on_close_requested(self) -> None:
        ...
        try:
            ...
            # Proactively ask Qt to quit as well; presentation layer also quits
            app = QApplication.instance()
            if app is not None:
                app.quit()
        except Exception:
