...
from src.domain.audio.entities.audio_recorder import AudioRecorder
    ...
    def _initialize_audio_to_text(self) -> None:
        ...
        try:
            ...
            if transcriber_available and vad_available:
                ...
                from src.application.listener.audio_to_text_config import (
                    AudioToTextConfig,
                )
                from src.application.listener.audio_to_text_service import (
                    AudioToTextService,
                )
                config = AudioToTextConfig(
                    ...
                    channels=1,
                    rate=16000,
                    start_sound_file="@resources/splash.wav",
                )
        ...
        try:
            with self._op_lock:
                if self._starting or self._stopping:
                    return
                self._starting = True
            # Trigger underlying start (plays beep immediately in service)
            if self._original_start_recording:
                self._original_start_recording()

            # Proceed only if low-level service actually entered recording state
            started = False
            try:
                started = bool(getattr(self._audio_to_text, "is_recording", False))
            except Exception:
                started = False

            if started:
                # Update DDD entity state only when low-level start succeeded
                ...
                def _post_check() -> None:
                    try:
                        if not self.check_audio_device():
                            # Emit a single friendly UI message and avoid extra console warnings
                            if self._ui_callback:
                                self._ui_callback("No recording device detected. Please connect a microphone.", None, None, None, None)
                            if self._logger:
                                self._logger.log_debug("Recording stopped: no input device detected")
                            with contextlib.suppress(Exception):
        ...
        try:
            ...
            if self._original_stop_recording:
                try:
                    ...
                    if self._logger:
                        self._logger.log_debug(f"Original stop_recording raised: {e}")
