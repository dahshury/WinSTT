...
from src.presentation.qt.services.drag_drop_service import (
    DragDropManager,
    DragDropService,
)

if TYPE_CHECKING:
    from src.presentation.qt.services.drag_drop_integration_service import (
...
class QtDragDropAdapter(IDragDropPort):
    ...
    def __init__(self, logger: LoggingPort | None = None):
        ...
        # Share the same service instance with the manager so emitted signals reach the connected callbacks
        self._drag_drop_manager = DragDropManager(service=self._drag_drop_service)
    ...
    def enable_widget_drag_drop(self, widget, zone_id: str, media_files: bool = True) -> Result[None]:
        ...
        try:
            ...
            # Remember widget and create a single overlay frame for visual feedback
            self._drop_zones[zone_id]["widget"] = widget
            overlay = QFrame(widget)
            overlay.setObjectName("dropOverlay")
            overlay.setStyleSheet(
                "QFrame#dropOverlay { border: 2px dashed rgb(96, 125, 139); border-radius: 6px; background: rgba(0,0,0,0); }",
            )
            overlay.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
            overlay.hide()
            self._drop_zones[zone_id]["_overlay"] = overlay

            # Provide visual feedback on drag enter/leave using shared service signals (overlay only)
            ...
            # Prevent duplicate connections causing multiple highlights
            if not self._drop_zones[zone_id].get("_connected", False):
                with suppress(Exception):
                    self._drag_drop_service.drag_entered.connect(lambda _e: _apply_highlight())
                    self._drag_drop_service.drag_left.connect(lambda: _remove_highlight())
                    self._drag_drop_service.files_dropped.connect(lambda _e: _remove_highlight())
                    self._drag_drop_service.invalid_files_dropped.connect(lambda _e: _remove_highlight())
                    self._drop_zones[zone_id]["_connected"] = True
