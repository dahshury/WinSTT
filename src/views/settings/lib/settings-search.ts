...
export function useSettingsSearchKeywords(): Record<string, string> {
	...
	return {
		...
		model: [
			tm("mainModel"),
			tm("realtimePreviewSection"),
			tm("realtimeModel"),
			tm("localAcceleration"),
			tm("computeDevice"),
			tm("language"),
			tm("translateTo"),
			tm("modelMemory"),
		...
		vocabulary: [
			...
			tSnip("trigger"),
			tSnip("expansion"),
			tDict("aiHandlingTitle"),
			tDict("llmHandlesDictionaryLabel"),
			tDict("llmHandlesSnippetsLabel"),
			"dictionary snippets vocabulary replacement expansion ai handling llm offload",
		].join(" "),
		output: [
