...
export function TextureSeparator({ className }: { className?: string }) {
	return (
		<div
			aria-hidden="true"
			className={cn("relative h-[2px] w-full", className)}
		>
...
export interface TextureCardRegionProps
...
export function TextureCardHeader({
	...
	children,
	...props
}: TextureCardRegionProps) {
	return (
		<header
			className={cn("flex items-center gap-3 px-5 pt-4 pb-4", className)}
			{...props}
		>
			{children}
		</header>
...
export function TextureCardBody({
	className,
	children,
	...props
}: TextureCardRegionProps) {
