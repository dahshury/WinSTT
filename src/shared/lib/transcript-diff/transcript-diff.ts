...
/**
 * Reconstruct a transcript from a diff with some changes rejected. Used by the
 * preview "Enhance" cherry-pick: walks the `before → after` hunks and, for every
 * CHANGE hunk (indexed by its change ordinal — the same order as
 * {@link TranscriptDiffResult.changes}), keeps the AI's `after` wording unless
 * that ordinal is in `rejected`, in which case the original `before` wording is
 * restored. Equal hunks always pass through.
 *
 * NOTE: like the word-level diff itself, this rejoins on single spaces, so it
 * normalizes the transcript's internal whitespace. Callers that accept EVERY
 * change should paste the exact `after` string instead of calling this (see the
 * preview store's `applyEnhancement`) — only cherry-picked partials need the
 * reconstruction, and word-level granularity is what the diff UI exposes anyway.
 */
export function reconstructFromDiff(
	before: string,
	after: string,
	rejected: ReadonlySet<number> | readonly number[],
): string {
	...
	const rejectedSet =
		rejected instanceof Set ? rejected : new Set<number>(rejected);
	const parts: string[] = [];
	let changeIndex = 0;
	for (const hunk of diff.hunks) {
		if (hunk.kind === "equal") {
			if (hunk.before) {
				parts.push(hunk.before);
			}
			continue;
		}
		const text = rejectedSet.has(changeIndex) ? hunk.before : hunk.after;
		changeIndex += 1;
		if (text) {
			parts.push(text);
		}
