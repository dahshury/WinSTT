...
describe("resolvePublicAsset", () => {
	test("non-file origin (dev server) returns the absolute path unchanged", () => {
		expect(resolvePublicAsset("/icon.ico", "http:", "http://localhost:3000/")).toBe("/icon.ico");
		expect(
			resolvePublicAsset("/provider-icons/openai.png", "https:", "https://localhost:3000/")
	...
	test("file:// main window resolves against the renderer root", () => {
		expect(resolvePublicAsset("/icon.ico", "file:", "file:///C:/app/renderer/index.html")).toBe(
			"file:///C:/app/renderer/icon.ico"
		);
	});
