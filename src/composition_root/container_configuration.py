...
from src.application.application_config import (
    ApplicationConfiguration,
    create_default_configuration,
)
from src.application.application_orchestrator import (
    ApplicationOrchestrator,
    create_application_orchestrator,
)

# Note: Shutdown use case temporarily disabled due to missing implementation
# from src.application.application_lifecycle.shutdown_application_use_case import (
#     ShutdownApplicationUseCase,
#     create_shutdown_use_case,
# )
from src.application.services.application_startup_service import (
...
from src.infrastructure.common.event_bus import (
    EventBus,
    EventBusManager,
    get_event_bus,
...
from src.infrastructure.common.task_manager import (
    TaskManager,
    create_task_manager,
)
from src.infrastructure.common.unit_of_work import (
    create_in_memory_unit_of_work,
)
from src.presentation.core.container import (
    EnterpriseContainer,
    EnterpriseContainerBuilder,
)
from src.presentation.infrastructure_bridge.ui_core_abstractions import (
    UIEventBus as UIEventBusCore,
)
from src.presentation.infrastructure_bridge.ui_core_abstractions import (
    UILifecycleManager,
)
from src.presentation.infrastructure_bridge.ui_core_patterns import (
    UIAnimationManager,
    UILayoutManager,
    UIPatternRegistry,
    UIThemeManager,
...
class ContainerConfiguration:
    ...
    def _register_infrastructure_services(self) -> None:
        ...
        if self._builder is not None:
            from src.infrastructure.common.unit_of_work import InMemoryUnitOfWork
    ...
    def _create_application_startup_service(self) -> ApplicationStartupService:
        ...
        from src.infrastructure.adapters.logging_adapter import PythonLoggingAdapter
        from src.infrastructure.adapters.minimal_adapters import (
...
def register_worker_services(builder: EnterpriseContainerBuilder) -> None:
    ...
    # Use refactored worker services instead of legacy workers
    from src.infrastructure.audio.listener_worker_service import (
        ListenerWorkerService,
    )
    from src.infrastructure.audio.vad_worker_service import (
        VadWorkerService,
    )
    from src.infrastructure.llm.llm_pyqt_worker_service import (
        LLMPyQtWorkerService,
    )
    from src.infrastructure.transcription.model_worker_service import (
        ModelWorkerService,
    )

    # Register worker classes as transient for thread-based operations
    builder.add_transient(VadWorkerService, VadWorkerService)
    builder.add_transient(ModelWorkerService, ModelWorkerService)
    builder.add_transient(ListenerWorkerService, ListenerWorkerService)
    builder.add_transient(LLMPyQtWorkerService, LLMPyQtWorkerService)
...
def register_ui_adapters(builder: EnterpriseContainerBuilder) -> None:
    ...
    """
    from src.infrastructure.adapters.pyqt6.widget_adapters import QtUIWidgetFactory
    from src.presentation.core.ui_abstractions import IUIWidgetFactory
...
def register_ui_patterns(builder: EnterpriseContainerBuilder) -> None:
    ...
    from src.presentation.core.patterns import (
        AnimationContext,
        FadeInStrategy,
        IWidgetFactory,
        LoggingDecorator,
