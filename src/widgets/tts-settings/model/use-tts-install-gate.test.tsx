...
function makeState(id: string, state: "cached" | "partial" | "not_cached"): TtsModelStateEntry {
	return {
		id,
		effectiveQuantization: "fp32",
		estimatedBytes: 0,
		cacheByQuantization: {
			fp32: { state, downloadedBytes: 0, totalBytes: 0, progress: state === "cached" ? 1 : 0 },
		},
	};
}
...
beforeEach(() => {
	...
	useSettingsStore.setState({
		settings: { ...DEFAULT_SETTINGS, tts: { ...DEFAULT_SETTINGS.tts, model: TEST_MODEL } },
		isLoaded: true,
	});
});
...
describe("buildTtsEnablePatch", () => {
	...
	test("empty current hotkey → folds in the default binding", () => {
		expect(buildTtsEnablePatch("", "LWin+R")).toEqual({ enabled: true, hotkey: "LWin+R" });
	});
