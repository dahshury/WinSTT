...
function RowActions({ item, ...h }: { item: FileQueueItem } & RowActionHandlers) {
	const t = useTranslations("fileOverlay");
		...
		<>
			...
			{item.status === "paused" && (
				<button
					...
					className={cn(PRIMARY_BTN, "text-teal/90 hover:text-teal focus-visible:ring-teal/60")}
			...
			{item.status === "error" && (
				<button
					...
					className={cn(PRIMARY_BTN, "text-error/80 hover:text-error focus-visible:ring-error/60")}
...
export function QueueRow({ item, index, rowLevel, ...handlers }: QueueRowProps) {
	...
	const determinate =
		isActive || item.status === "complete" || item.status === "paused" || hasProgress;
	...
	return (
		<li
			...
			className={cn(
				...
				isActive
					? cn(surfaceBg(rowLevel), "shadow-[inset_2px_0_0_0_var(--color-teal)]")
					: "bg-transparent"
			)}
			// Per-row entrance stagger, capped so a large drop still settles fast.
			style={{ animationDelay: `${Math.min(index, 6) * 45}ms` }}
		>
			<span aria-hidden className="grid size-[14px] shrink-0 place-items-center">
