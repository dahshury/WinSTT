...
export const DEFAULT_LLM: LlmDraftSnapshot = {
	...
	transforms: {
		...DEFAULT_FEATURE,
		...DEFAULT_NEUTRAL_PRESET_CARRIER,
		hotkey: "",
	},
};
...
function isNonEmptyPresetList(
	value: unknown,
): value is readonly BuiltinPresetEntry[] {
...
function resolvePresets(
	...
	return isNonEmptyPresetList(src.presets) ? src.presets : fallback.presets;
}
...
function readDictationDraft(
	dictationIn: Partial<LlmDictation>,
...
function unwrapLlm(
	llm: Partial<LlmSettings> | null | undefined,
): Partial<LlmSettings> {
	return llm ?? {};
}

function unwrapDictation(
	incoming: Partial<LlmSettings>,
): Partial<LlmDictation> {
	return (incoming.dictation ?? {}) as Partial<LlmDictation>;
}

function unwrapTransforms(
	incoming: Partial<LlmSettings>,
): Partial<LlmTransforms> {
	return (incoming.transforms ?? {}) as Partial<LlmTransforms>;
}

export function readLlmSnapshot(
	llm: Partial<LlmSettings> | null | undefined,
): LlmDraftSnapshot {
...
export function getToneKey(
	presets: readonly BuiltinPresetEntry[],
): (typeof TONE_GROUP)[number] {
	const tone = presets.find((p) =>
		(TONE_GROUP as readonly string[]).includes(p.key),
	);
...
export function setTone(
	...
	const withoutTone = presets.filter(
		(p) => !(TONE_GROUP as readonly string[]).includes(p.key),
	);
...
export function toggleIndependent(
	...
	return [
		...presets,
		makeIndependentEntry(key, levelOverride, targetLangOverride),
	];
}
...
function findFirstDifferentModel(
...
export function resolveOllamaModelReconcilePatch(
	provider: string,
	models: readonly OllamaModel[],
	current: string,
	enabled: boolean,
...
export function shouldScanOpenRouter(
	provider: string,
	apiKey: string,
	loaded: boolean,
): boolean {
...
export function pickSmallestInstalledOllama(
	models: readonly OllamaModel[],
): string | null {
...
function continueOllamaEnable(deps: FeatureToggleDeps): void {
	const paths: Readonly<
		Record<"keep" | "replace", (d: FeatureToggleDeps) => void>
	> = {
...
export async function tryEnableOllamaForFeature(
	deps: FeatureToggleDeps,
): Promise<void> {
	const reachable = await deps.checkOllamaReachable();
	const flows: Readonly<
		Record<"reachable" | "unreachable", (d: FeatureToggleDeps) => void>
	> = {
...
function continueOpenRouterEnable(deps: FeatureToggleDeps): void {
	...
	const paths: Readonly<
		Record<"existing" | "default", (d: FeatureToggleDeps) => void>
	> = {
...
export function tryEnableOpenRouterForFeature(deps: FeatureToggleDeps): void {
	const flows: Readonly<
		Record<"hasKey" | "noKey", (d: FeatureToggleDeps) => void>
	> = {
...
export async function performFeatureToggle(
	next: boolean,
	deps: FeatureToggleDeps,
): Promise<void> {
	if (!next) {
...
export function getOllamaDialogTexts(
	showRun: boolean,
	t: TranslateFn,
): OllamaDialogTexts {
	if (showRun) {
