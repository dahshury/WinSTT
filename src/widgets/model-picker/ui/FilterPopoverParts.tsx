...
function SortChipsSection<TSortKey extends string>({
	hint,
	icons,
	keys,
	labels,
...
/**
 * Sort + filter configuration for {@link FilterMenu}. Bundled so every picker's
 * "Sort by" chip row is described the same way (the STT / Ollama menus differ
 * only in their keys / labels / icons, never in the chip mechanics).
 */
interface FilterSortConfig<TSortKey extends string> {
	hint: string;
	icons: Record<TSortKey, IconSvgElement>;
	keys: readonly TSortKey[];
	labels: Record<TSortKey, string>;
