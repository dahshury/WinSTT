...
import { formatContextTokens } from "../../lib/model-spec-format";
import {
...
export function buildOllamaSpec(
	...
	const parameterSize = model.details?.parameterSize?.trim();
	if (parameterSize) {
		facts.push({
			key: "params",
			label: "Parameters",
	...
	const quant = model.details?.quantizationLevel?.trim();
	if (quant) {
		facts.push({
			key: "quant",
			label: "Quantization",
