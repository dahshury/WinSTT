...
from src.domain.ui_coordination.value_objects.opacity_effects import (
    EffectType,
    ManagePhase,
    ManageResult,
    OpacityMode,
        ...
        try:
            ...
            if state.state_backup and backup_created:
                try:
                    ...
                    self._logger.log_error("Failed to restore state from backup", error=restore_error)
        ...
        try:
            ...
            for target in request.target_widgets:
                ...
                if not self._widget_validation.validate_opacity_value(target.target_opacity):
                    state.errors.append(f"Invalid target opacity for widget {target.widget_id}: {target.target_opacity}")
                    return False

            # Validate animation configuration if provided
            if request.animation_config:
                if request.animation_config.duration_ms <= 0:
                    state.errors.append(f"Invalid animation duration: {request.animation_config.duration_ms}")
                    return False
        ...
        try:
            ...
            for target in request.target_widgets:
                if not self._opacity_management.supports_opacity_effects(target.widget_id):
                    state.warnings.append(f"Widget {target.widget_id} has limited opacity effect support")
    ...
    request: ManageOpacityEffectsRequest, state: OpacityEffectsState,
        ...
        for target in sorted_targets:
            try:
                ...
                effect_result = self._opacity_management.apply_opacity(target.widget_id, target.target_opacity)
                ...
                self._logger.log_error(f"Failed to apply opacity to widget {target.widget_id}", error=e)
    ...
    request: ManageOpacityEffectsRequest, state: OpacityEffectsState,
        ...
        try:
            if request.animation_config:
                ...
                # Create animation
                animation_result = self._animation.create_opacity_animation(request.animation_config, target_ids)
                ...
                if animation_result.animation_created and animation_result.animation_id:
                    animation_started = self._animation.start_animation(animation_result.animation_id)
