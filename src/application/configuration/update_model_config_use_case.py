...
from src.domain.settings.value_objects.update_operations import (
    ModelCompatibility,
    UpdatePhase,
    UpdateResult,
)
    ...
    def execute(self, request: UpdateModelConfigRequest,
        ...
        try:
            ...
            if request.update.validate_compatibility:
                ...
                compatibility_result = self._model_validation.check_model_compatibility(model_to_update, quantization_to_update)
                if compatibility_result.is_success:
                    ...
                    if compatibility_result.value == ModelCompatibility.INCOMPATIBLE:
                        response.error_message = f"Model {model_to_update.value} is incompatible with quantization {quantization_to_update.value}"
                        response.result = UpdateResult.VALIDATION_FAILED
                        return response
                    if compatibility_result.value == ModelCompatibility.PARTIALLY_COMPATIBLE:
                        response.warnings.append(f"Model {model_to_update.value} has limited compatibility with quantization {quantization_to_update.value}")
            ...
            if request.update.force_restart_worker:
                worker_start_result = self._worker_management.start_worker_with_config(model_to_update, quantization_to_update)
