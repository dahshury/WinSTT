...
from src.domain.common.progress_callback import ProgressCallback
from src.domain.common.result import Result
from src.domain.common.value_object import ProgressPercentage
from src.domain.settings.value_objects.configuration_operations import (
    SaveFormat,
    SavePhase,
    SaveResult,
    SaveStrategy,
    ...
    def execute(self, request: SaveConfigurationRequest,
        ...
        try:
            ...
            if not path_validation.is_success:
                response.error_message = f"Invalid target path: {path_validation.error}"
                return response
            ...
            if estimated_size_result.is_success:
                ...
                if request.save_config.compress_output and estimated_size is not None:
                ...
                if estimated_size is not None:
                    ...
                    if estimated_size > max_size_bytes:
                        response.error_message = f"Estimated file size ({estimated_size} bytes) exceeds limit ({max_size_bytes} bytes)"
                        return response

                    # Check available disk space
                    available_space_result = self._file_system.get_available_space(request.save_config.target_path)
                    if available_space_result.is_success and available_space_result.value is not None and available_space_result.value < estimated_size * 2:
            ...
            if request.save_config.validate_before_save:
                validation_result = self._validation.validate_configuration_data(request.configuration_data)
            ...
            if request.save_config.exclude_sensitive_data:
                ...
                if sanitize_result.is_success and sanitize_result.value is not None:
                    ...
                    response.validation_warnings.append(f"Failed to sanitize sensitive data: {sanitize_result.error}")

            # Add metadata if requested
            if request.save_config.include_metadata:
                metadata = self._metadata.create_save_metadata(request.save_config, len(str(data_to_save)))
                data_to_save = {
                    "_metadata": metadata,
            ...
            backup_path = None
            if request.save_config.create_backup and self._file_system.file_exists(request.save_config.target_path):
                if not self._update_progress(request.progress_callback, SavePhase.CREATING_BACKUP, 30):
                    response.result = SaveResult.CANCELLED
                    return response

                backup_path = f"{request.save_config.target_path}{request.save_config.backup_suffix}"
                backup_result = self._file_system.copy_file(request.save_config.target_path, backup_path)
                if backup_result.is_success:
                    ...
                    response.validation_warnings.append(f"Failed to create backup: {backup_result.error}")
            ...
            if not serialized_data.is_success:
                response.error_message = f"Serialization failed: {serialized_data.error}"
            ...
            if request.save_config.compress_output and final_data is not None:
                if isinstance(final_data, str):
                    final_data = final_data.encode("utf-8")
                compress_result = self._serialization.compress_data(final_data)
                if compress_result.is_success and compress_result.value is not None:
                    original_size = len(final_data)
                    final_data = compress_result.value
                    response.compression_ratio = len(final_data) / original_size
                else:
                    response.validation_warnings.append(f"Compression failed: {compress_result.error}")

            # Apply encryption if requested
            if request.save_config.encryption_key and final_data is not None:
                if isinstance(final_data, str):
                    final_data = final_data.encode("utf-8")
                encrypt_result = self._serialization.encrypt_data(final_data, request.save_config.encryption_key)
                if encrypt_result.is_success and encrypt_result.value is not None:
                    final_data = encrypt_result.value
                else:
                    response.validation_warnings.append(f"Encryption failed: {encrypt_result.error}")
            ...
            # Ensure target directory exists via file system service (no direct os)
            target_dir = request.save_config.target_path.rsplit("/", 1)[0] if "/" in request.save_config.target_path else ""
            if target_dir:
                dir_result = self._file_system.create_directory(target_dir)
                if not dir_result.is_success:
                    response.error_message = f"Failed to create target directory: {dir_result.error}"
                    return response
            ...
            if final_data is not None:
                ...
                write_result = Result.failure("Final data is None")
            if not write_result.is_success:
                response.error_message = f"Write operation failed: {write_result.error}"
                ...
                if backup_path and request.save_config.strategy == SaveStrategy.ATOMIC:
                    restore_result = self._file_system.copy_file(backup_path, request.save_config.target_path)
                    if not restore_result.is_success:
                        response.validation_warnings.append(f"Failed to restore backup: {restore_result.error}")
                return response
            ...
            if request.save_config.verify_after_save:
                ...
                if not verify_result.is_success:
                    response.validation_warnings.append(f"Save verification failed: {verify_result.error}")
            ...
            history_result = self._metadata.update_save_history(request.save_config.target_path, save_metadata)
            if not history_result.is_success:
                response.validation_warnings.append(f"Failed to update save history: {history_result.error}")
    ...
    def _serialize_data(self, data: dict[str, Any], config: SaveConfiguration,
        ...
        try:
            if config.format == SaveFormat.JSON:
                result = self._serialization.serialize_to_json(data, config.pretty_format)
                if result.is_success:
                    return Result.success(result.value)  # type: ignore[arg-type]
                return Result.failure(result.error or "Serialization failed")
            if config.format == SaveFormat.XML:
                result = self._serialization.serialize_to_xml(data, config.pretty_format)
                if result.is_success:
                    return Result.success(result.value)  # type: ignore[arg-type]
                return Result.failure(result.error or "Serialization failed")
            if config.format == SaveFormat.YAML:
                result = self._serialization.serialize_to_yaml(data)
                if result.is_success:
                    return Result.success(result.value)  # type: ignore[arg-type]
                return Result.failure(result.error or "Serialization failed")
            if config.format == SaveFormat.INI:
                result = self._serialization.serialize_to_ini(data)
                if result.is_success:
                    return Result.success(result.value)  # type: ignore[arg-type]
                return Result.failure(result.error or "Serialization failed")
            if config.format == SaveFormat.BINARY:
                binary_result = self._serialization.serialize_to_binary(data)
                if binary_result.is_success:
                    return Result.success(binary_result.value)  # type: ignore[arg-type]
                return Result.failure(binary_result.error or "Serialization failed")
    ...
    def _write_data_with_strategy(self, data: bytes | str, target_path: str, strategy: SaveStrategy,
        ...
        try:
            ...
            if strategy == SaveStrategy.ATOMIC:
                # Write to temporary file first, then rename via file system service
                temp_path = f"{target_path}.tmp"
                write_result = self._file_system.write_file(temp_path, data)
                if write_result.is_success:
                    rename_result = getattr(self._file_system, "move_file", None)
                    if callable(rename_result):
                        mv_result = self._file_system.move_file(temp_path, target_path)
                        if mv_result.is_success:
                            return Result.success(None)
                        self._file_system.delete_file(temp_path)
                        return Result.failure(f"Atomic rename failed: {mv_result.error}")
                    # Fallback: delete temp and fail if move_file not provided
                    self._file_system.delete_file(temp_path)
                    return Result.failure("File system service missing move_file for atomic strategy")
                return write_result
            ...
            if strategy == SaveStrategy.VERSIONED:
                # Create versioned filename without direct os calls
                dot_index = target_path.rfind(".")
                if dot_index != -1:
                    base, ext = target_path[:dot_index], target_path[dot_index:]
                else:
                    base, ext = target_path, ""
    ...
    saved_path: str, original_data: dict[str, Any], config: SaveConfiguration,
        ...
        try:
            ...
            if not read_result.is_success:
                return Result.failure(f"Could not read saved file: {read_result.error}")

            # For basic verification, just check that the file exists and has content
            saved_content = read_result.value
            if saved_content is not None and len(saved_content) == 0:
