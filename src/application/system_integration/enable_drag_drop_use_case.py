...
from src.domain.application_lifecycle.value_objects import EnablePhase
from src.domain.file_operations.value_objects import (
    DropAction,
    DropZoneType,
    FileType,
    ProcessingMode,
...
class DragDropState:
    ...
    drop_zone_results: list[DropZoneSetupResult] = field(default_factory=list)
    file_type_results: list[FileTypeSetupResult] = field(default_factory=list)
...
class EnableDragDropResponse:
    ...
    phase_times: dict[EnablePhase, float] = field(default_factory=dict)
...
class EnableDragDropUseCase:
    ...
    def execute(self, request: EnableDragDropRequest,
        ...
        try:
            ...
            for file_type_config in request.file_types:
                file_type_result = FileTypeSetupResult(
                    ...
                    mime_types_registered=(
                        len(file_type_config.mime_types or []) if types_registered else 0
                    ),
                    validator_installed=(
            ...
            # Register event callbacks
            callbacks_registered, callbacks_count, callbacks_error = self._event_binding_service.register_event_callbacks(
