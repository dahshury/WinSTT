...
export interface SttModelSelectorTriggerProps {
	...
	/** True while the selected model is loading or warming at app startup.
	 * Separate from swap state so the control says Loading, not Switching. */
...
function buildAriaLabel(inputs: AriaLabelInputs): string | undefined {
	...
	if (inputs.isModelPreparing && !inputs.isSwitching) {
		return `Loading ${inputs.selectedModel?.displayName ?? "speech model"}. Transcription will be ready shortly.`;
	}
...
function TriggerButton({ buttonProps, ...rest }: TriggerButtonProps) {
	...
	const isTriggerActive =
		isSwitching || isBackgroundDownload || isModelPreparing;
	...
	return (
		...
		>
			...
			<TriggerBody
				...
				toModel={toModel}
			/>
			{isModelPreparing && !(isSwitching || isBackgroundDownload) ? (
