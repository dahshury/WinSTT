...
export interface SttModelSelectorProps {
	...
	inline?: boolean;
	isLoading?: boolean;
	/** The selected local model is loading weights or warming inference
	 * kernels. This is visual readiness state, not a hard interaction lock. */
	 ...
	 *  per-quant fit against the cross-modality memory budgets + language
	 *  rule). `undefined` = the host has no verdict yet (budgets not loaded)
	 *  or doesn't participate (onboarding) — the Suggested filter is then
	 *  inert and its chip hidden. Threaded the same way as
	 *  {@link SttModelSelectorProps.getFitAssessment}. */
...
const bySuggestionName = makeNameComparator((m: ModelInfo) => m.displayName);

/** The Suggested flat column: best bang-for-buck first (the engine's headline
 *  score = weighted harmonic mean of effective accuracy × speed at the best
 *  fitting quant), name A→Z on ties. Models without a verdict sink to the end. */
...
function useSttModelSelectorPanelState({
	...
	// unknown-footprint rule of the legacy hardware filter).
	const suggestionsAvailable = getSuggestion !== undefined;
	const suggestedPredicate = getSuggestion
		? (m: ModelInfo) => getSuggestion(m.id)?.visible !== false
		: null;
	const suggestedFilterOn =
		suggestionsAvailable && effectiveFilters.suggestedOnly;
	// Menu filters (cached / realtime / language / hardware / suggested) prune
	// the items before the picker sees them; the text query is then handled by
	// Base UI's `filter` (via the shell) so groups + keyboard nav stay consistent.
	const filterCtx: FilterModelsContext = {
	...
	const menuFilteredModels = shouldBuildList
		? filterSttModels(baseModels, filterCtx)
		: [];
	...
	const suggestedHiddenCount =
		shouldBuildList && suggestedFilterOn
			? filterSttModels(baseModels, {
					...filterCtx,
					filters: { ...effectiveFilters, suggestedOnly: false },
				}).length - menuFilteredModels.length
			: 0;
	...
	const allGroups: SttListGroup[] = suggestedFlattenActive
		? [
				{
					value: SORTED_GROUP_VALUE,
					items: sortByBangForBuck(menuFilteredModels, getSuggestion),
				},
			]
		: sort === null
	...
	const viewProps: SttModelSelectorViewProps = {
		...
		getFitAssessment,
		// Per-quant badge gating: while Suggested is ON, badges outside the
		// fitting set render disabled/greyed. Keyed by the badge's BACKING model
		// id so streaming-precision-merged cards gate each precision against the
		// raw catalog row that actually publishes it.
		getFittingQuants:
			suggestedFilterOn && getSuggestion
				? (modelId: string) => getSuggestion(modelId)?.fittingQuants ?? null
				: undefined,
		suggestedFlattenActive,
		suggestedHiddenCount,
		suggestionsAvailable,
		onShowAllSuggested: () =>
			dispatch({
				type: "setFilters",
				filters: applyLockedFilters(
					{ ...effectiveFilters, suggestedOnly: false },
					lockedFilterKeys,
				),
			}),
		canDeleteQuant,
