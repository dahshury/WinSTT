...
// Intl-wrapped render: the card's PrecisionGroup reads `useTranslations`
// (the Suggested "needs more memory" tooltip), so the raw RTL render throws.
import {
	fireEvent,
	render,
	screen,
...
describe("Suggested per-quant badge gating", () => {
	const CACHED = {
		state: "cached",
		progress: 1,
		downloaded_bytes: 10,
	...
	function renderGatedCard({
		fittingQuants,
		state,
	}: {
		fittingQuants: ReadonlySet<string> | null;
		state?: import("@/shared/api/ipc-client").ModelStateEntry;
	}) {
		const onSelect = mock(() => undefined);
		const onDownloadAction = mock(() => undefined);
		const model = makeModel({ availableQuantizations: ["", "fp16", "int8"] });
					...
					<Combobox.List>
						{() => (
							<SttModelCard
								currentQuantization=""
								getDownloadSnapshot={() => undefined}
								getFittingQuants={() => fittingQuants}
								key={model.id}
								model={model}
								onDownloadAction={onDownloadAction}
								onSelect={onSelect}
		...
		return { onSelect, onDownloadAction };
	}

	test("badges outside the fitting set render disabled and ignore clicks", () => {
		// fp32 doesn't fit the budgets → its badge is aria-disabled, greyed, and
		// no longer a click-to-download trigger; the fitting int8 badge stays live.
		const { onDownloadAction } = renderGatedCard({
			fittingQuants: new Set(["int8"]),
		});
		const fp32 = screen.getByLabelText("Select fp32 precision");
		expect(fp32.getAttribute("aria-disabled")).toBe("true");
		fireEvent.click(fp32);
		expect(onDownloadAction).not.toHaveBeenCalled();
		// int8 fits → still the ordinary uncached download trigger.
		const int8 = screen.getByLabelText("Download int8 weights");
		expect(int8.getAttribute("aria-disabled")).toBe("false");
		fireEvent.click(int8);
		expect(onDownloadAction).toHaveBeenCalledWith(
			"start",
			"custom-my-whisper",
			"int8",
	...
	test("a disabled CACHED badge no longer selects but keeps its delete control", () => {
		const onSelect = mock(() => undefined);
		const model = makeModel({ availableQuantizations: ["", "int8"] });
		const onRequestDeleteQuant = mock(() => undefined);
		render(
			<TooltipProvider.Provider>
				<Combobox.Root items={[model]}>
					<Combobox.List>
		...
		const int8 = screen.getByLabelText("Select int8 precision");
		expect(int8.getAttribute("aria-disabled")).toBe("true");
		fireEvent.click(int8);
		expect(onSelect).not.toHaveBeenCalled();
		// The cached-on-disk delete affordance survives the gating.
		fireEvent.click(
			screen.getByLabelText("Delete int8 weights for My Whisper"),
		);
		expect(onRequestDeleteQuant).toHaveBeenCalled();
	});

	test("the Recommended mark stays on the effective quant inside the fitting set", () => {
		// The server's fit-aware effective_quantization is inside the fitting set
		// by construction — the sparkle/aria mark must survive Suggested gating,
		// and the unfit fp32 badge must not steal or lose marks it never had.
		renderGatedCard({
			fittingQuants: new Set(["fp16", "int8"]),
			state: makeState({ effective_quantization: "fp16" }),
		});
		const fp16 = screen.getByLabelText(
			"Download fp16 weights (recommended for your hardware)",
		);
		expect(fp16.getAttribute("aria-disabled")).toBe("false");
		expect(
			screen
				.getByLabelText("Select fp32 precision")
				.getAttribute("aria-disabled"),
		).toBe("true");
	});

	test("a null fitting set (no verdict for this model) disables nothing", () => {
		const { onDownloadAction } = renderGatedCard({ fittingQuants: null });
		fireEvent.click(screen.getByLabelText("Download fp32 weights"));
		expect(onDownloadAction).toHaveBeenCalledWith(
			"start",
			"custom-my-whisper",
			"",
