...
import { SttFiltersMenu, SuggestedFilterChip } from "./SttFiltersMenu";
...
export interface SttModelSelectorViewProps {
	...
	getFitAssessment?:
		...
		| undefined;
	/** Per-quant Suggested gating (defined only while the flag is ON and the
	 *  host wired a verdict): fitting quant set per BACKING model id, `null`
	 *  when that model has no verdict. */
	getFittingQuants?:
		| ((modelId: string) => ReadonlySet<string> | null)
		| undefined;
	/** Suggested ON + no explicit sort → the flat bang-for-buck column. */
	suggestedFlattenActive: boolean;
		...
		<ModelPicker<ModelInfo, ModelInfo | null>
			...
			list={
				<SttModelList
					...
					hasSearch={inputValue.trim().length > 0}
