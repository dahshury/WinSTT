...
describe("getQuantizationOptions", () => {
	...
	test("orders by precision weight, not the canonical list (fp32 then fp16 before int8)", () => {
		...
		const opts = getQuantizationOptions(
			model({
				availableQuantizations: ["q4", "int8", "fp16", "", "q4f16", "int4"],
			}),
		);
	...
	test("keeps int4-only models visible as a selectable precision", () => {
		const opts = getQuantizationOptions(
			model({
				id: "qwen3-asr-0.6b",
		...
		expect(opts.map((o) => o.value)).toEqual(["int4"]);
		expect(opts[0]?.label).toBe("int4");
		expect(opts[0]?.tooltip).toContain("4-bit");
	});
