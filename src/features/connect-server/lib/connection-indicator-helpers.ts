...
export function resolveGpuChipConfig(isGpu: boolean): GpuChipConfig {
	return isGpu
		? { icon: GpuIcon, label: "GPU", colorClass: "text-foreground" }
 ...
 *
 * "connected" on the WebSocket alone is NOT enough to show the ready
 ...
 * ready chip while the recorder is still cold.
 *
