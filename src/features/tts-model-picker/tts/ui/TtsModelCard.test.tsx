...
// The card now consumes `useTranslations` (the Suggested "needs more memory"
// tooltip), so the raw RTL render would throw — same move as SttModelCard.
import {
	fireEvent,
	render,
	screen,
...
function renderCard(opts: {
	...
	const getFittingQuants =
		opts.fittingQuants === undefined
			? undefined
			: () => opts.fittingQuants ?? null;
	const utils = render(
...
describe("Suggested per-quant badge gating", () => {
	const twoQuantModel = () =>
		makeModel({
			...
			sizeBytesByQuantization: { fp16: 169_869_312, int8: 84_934_656 },
		});

	test("a precision outside the fitting set is disabled and never downloads", () => {
		const { onDownloadAction, onSelect } = renderCard({
			fittingQuants: new Set(["int8"]),
			model: twoQuantModel(),
			state: makeState("not_cached"),
		});
		// The unfit badge stops advertising a download (label aria, not
		// "Download fp16 weights") and rejects the click outright.
		const unfit = screen.getByLabelText("Select fp16 precision");
		expect(unfit.getAttribute("aria-disabled")).toBe("true");
		fireEvent.click(unfit);
		expect(onDownloadAction).not.toHaveBeenCalled();
		expect(onSelect).not.toHaveBeenCalled();
		// The fitting sibling keeps its normal download affordance.
	...
	test("no verdict (null) leaves every badge interactive", () => {
		const { onDownloadAction } = renderCard({
			fittingQuants: null,
			model: twoQuantModel(),
			state: makeState("not_cached"),
		});
		fireEvent.click(screen.getByLabelText("Download fp16 weights"));
		expect(onDownloadAction).toHaveBeenCalledWith(
			"start",
			"kokoro-82m",
			"fp16",
