...
/** Host-injected Suggested verdict (spec-based recommender). Structural mirror
 *  of `OllamaSuggestions` in `features/suggested-models` so the picker stays
 *  presentational: `fits` is the budget-aware per-tag test (either-pool — VRAM
 *  budget OR RAM budget; the VRAM-only rule stays with the `systemFit` warning
 *  chip), `score` the bang-for-buck proxy used to rank recommended cards while
 *  the flag is ON. `undefined` = the host has no verdict (system info not
 *  loaded, onboarding) — the Suggested chip is hidden and the flag inert. */
export interface OllamaSuggestionsProp {
...
export interface OllamaModelSelectorProps {
	...
	swap?:
		...
		| null
		| undefined;
	/** Host-computed Suggested verdict — see {@link OllamaSuggestionsProp}.
	 *  Threaded the same way as the STT picker's `getSuggestion`. */
...
export interface QuantShelfDeps {
	...
	/** Suggested per-tag gating (set only while the flag is ON and the host
	 *  wired a verdict): a budget-aware fit test — tags outside it render
	 *  disabled/greyed with the "needs more memory" tooltip. */
	suggestedFits?: ((sizeBytes: number) => boolean) | undefined;
}
