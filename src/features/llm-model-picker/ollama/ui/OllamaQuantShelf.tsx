...
import { QuantShelf } from "@/shared/ui/model-picker/core/model-card/QuantShelf";
import type {
	QuantDownloadAction,
	QuantDownloadSnapshot,
	QuantShelfEntry,
} from "@/shared/ui/model-picker/core/model-card/quant-shelf-types";
...
function buildOllamaQuantEntries({
	...
	| "tags"
> & {
	/** Localized "needs more memory" tooltip for Suggested-gated tags. */
	unfitReason: string;
}): QuantShelfEntry[] {
	return tags.map((tag) => {
		...
		// Suggested per-tag gating: while the flag is ON (`suggestedFits` set), a
		// tag outside the budget-aware fitting set renders disabled/greyed and
		// stops advertising a download — mirroring the STT shelf's per-quant
		// gating. Unknown sizes stay lenient (never gate what we can't judge).
		const unfit =
			suggestedFits !== undefined &&
			fullSizeBytes !== null &&
			fullSizeBytes > 0 &&
			!suggestedFits(fullSizeBytes);
		return {
			...
			canResumeDownload: isPaused && !unfit,
			canStartDownload: canStartDownload && !unfit,
...
interface OllamaQuantShelfProps {
	...
	/** Budget-aware Suggested fit test (set only while the flag is ON): tags
	 *  whose size fails it render disabled with the "needs more memory"
	 *  tooltip. Distinct from `getFit` (the VRAM-first warning chip rule). */
...
export function OllamaQuantShelf({
	...
	const entries = dedupeOllamaQuantEntries(
		buildOllamaQuantEntries({
			...
			suggestedFits,
			tags: visibleTags,
			unfitReason: t("quantNeedsMoreMemory"),
		}),
	);
	return (
