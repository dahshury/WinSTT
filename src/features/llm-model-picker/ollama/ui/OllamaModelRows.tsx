...
import { FAVORITES_GROUP_VALUE } from "@/shared/ui/model-picker/core/favorites";
import type { MetaEntry } from "@/shared/ui/model-picker/core/model-card/CardMeta";
import {
	GroupHeader,
	NeutralHeaderIcon,
} from "@/shared/ui/model-picker/core/model-card/GroupHeader";
import { ModelCard } from "@/shared/ui/model-picker/core/model-card/ModelCard";
import {
	Tooltip,
	TooltipContent,
	TooltipTrigger,
} from "@/shared/ui/model-picker/ui/Tooltip";
import {
...
function buildInstalledMetaEntries(model: OllamaModel): MetaEntry[] {
	...
	const paramSize = resolveOllamaParameterSize(model);
	if (paramSize) {
		entries.push({
			key: "params",
			icon: Atom01Icon,
...
function OllamaModelRow({
	...
	const activePullName = shelfDeps
		? activePullNameForRow(
				shelfDeps.pulls,
				model.name,
				resolveOllamaParameterSize(model) ?? paramSizeFromName(model.name),
			)
		: null;
	return (
		<ModelCard
	...
	return (
		<div className="flex flex-col items-center gap-1 p-6 text-center">
			...
			{showSuggestedHint ? (
				<BaseButton
					className={cn(
						"cursor-pointer text-balance text-accent text-xs underline-offset-2 outline-none",
...
function RecommendedRow({
	...
	const placeholder = (
		<OllamaQuantShelf
			...
			suggestedFits={shelfDeps.suggestedFits}
...
interface ListBodyProps {
	...
	onDelete: ((name: string) => void) | undefined;
	/** Turn the Suggested filter off — the empty-state hint's one-tap escape. */
	onShowAllSuggested?: (() => void) | undefined;
	onToggleFavorite: (name: string) => void;
	shelfDeps: QuantShelfDeps;
