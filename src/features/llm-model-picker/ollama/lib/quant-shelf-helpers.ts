 ...
 * q4_K_M size) · q4_K_M (sweet spot) · q5_K_M · q8_0 · fp16. GPT-OSS is a
 * special case: its official 20B/120B Ollama artifacts are natively MXFP4, but
 * Ollama's tag names omit the quant token, so those two tags are kept explicitly.
 *
...
const GPT_OSS_NATIVE_QUANT_RE = /^gpt-oss:(?:20b|120b)$/i;

function nativeQuantization(tag: OllamaLibraryTag): string | undefined {
	if (GPT_OSS_NATIVE_QUANT_RE.test(tag.name)) {
		return "MXFP4";
	}
	return undefined;
}

function isShownQuantTag(tag: OllamaLibraryTag): boolean {
	...
	// GPT-OSS has no alternative Ollama quant ladder, so hiding its native MXFP4
	// build would leave the card with no quant badge at all.
	return (
		CANONICAL_QUANT_RE.test(tag.name) || nativeQuantization(tag) !== undefined
	);
}
...
export function pruneToShownQuants(
	...
	return tags.filter((tag) => isShownQuantTag(tag) || forceKeep(tag.name));
}
...
export function quantBadgeLabel(tag: OllamaLibraryTag): string {
	...
	const native = nativeQuantization(tag);
	if (native) {
		return native;
	}
