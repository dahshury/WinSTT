...
describe("pruneToShownQuants", () => {
	...
	it("surfaces GPT-OSS native MXFP4 tags even though Ollama omits the quant token", () => {
		const tags = [
			tag({ name: "gpt-oss:latest", isLatest: true }),
			tag({ name: "gpt-oss:20b", parameterSize: "20B" }),
			tag({ name: "gpt-oss:120b", parameterSize: "120B" }),
		];
		const shown = pruneToShownQuants(tags, noKeep);
		expect(shown.map((candidate) => candidate.name)).toEqual([
			"gpt-oss:20b",
			"gpt-oss:120b",
		]);
		expect(shown.map(quantBadgeLabel)).toEqual(["MXFP4", "MXFP4"]);
	});
