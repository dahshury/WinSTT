import { AiMagicIcon, ArrowLeft01Icon, Cancel01Icon, Tick02Icon } from "@hugeicons/core-free-icons";
...
function selectedCustomModifierToRuntime(modifier: CustomModifier): CustomModifier {
	...
	return modifier.level === undefined ? base : { ...base, level: modifier.level };
}
...
function EnhanceView() {
	...
	const runEnhance = async () => {
		...
		if (s.scope === "selection" && s.source === "current" && s.selEnd > s.selStart) {
	...
	return (
		<div className="flex flex-col gap-2.5 px-1">
			<div className="flex items-center gap-2">
				...
				/>
				<span className="font-medium text-foreground text-sm">{tp("enhanceTitle")}</span>
			</div>
			...
			<div className="flex flex-wrap items-center gap-2">
				...
				<ElevatedSurface
					className={cn("transition-opacity", !scopeEnabled && "pointer-events-none opacity-40")}
				>
					<Switcher
...
function ReviewView() {
	...
	return (
		<div className="flex flex-col gap-2.5 px-1">
			<span className="font-medium text-foreground text-sm">{tp("reviewTitle")}</span>
...
/** The editable transcript + toolbar (default view). */
function EditView() {
	const tp = useTranslations("preview");
	const store = useTranscriptPreviewStore();
	const taRef = useRef<HTMLTextAreaElement>(null);
	const postProcessEnabled = useSettingsStore((s) => s.settings.llm?.dictation?.enabled ?? false);

	const syncSelection = () => {
		const el = taRef.current;
		if (el) {
			store.setSelection(el.selectionStart, el.selectionEnd);
		}
	};

	const openEnhance = () => {
		// Seed the configurator from the current dictation config so the default
		// "Run" reproduces the configured cleanup.
		const dictation = useSettingsStore.getState().settings.llm?.dictation;
		const presetKeys = (dictation?.presets ?? []).map((p) => p.key);
		...
		<div className="flex flex-col gap-2 px-1">
			<StaggerReveal>
				<textarea
					...
					dir="auto"
					onChange={(e) => store.setText(e.currentTarget.value)}
			...
			<div className="flex items-center justify-between">
				<IconButton
					aria-label={tp("enhance")}
					disabled={!postProcessEnabled}
					icon={<HugeiconsIcon icon={AiMagicIcon} size={16} />}
					onClick={openEnhance}
					tooltip={postProcessEnabled ? tp("enhance") : tp("enhanceDisabled")}
				/>
				<Button
