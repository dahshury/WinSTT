...
class DragDropService(QObject):
    ...
    def _handle_drag_enter(self, event: QDragEnterEvent,
        ...
        if valid_files:
            ...
            domain_event = DragDropEventData(
                files=valid_files,
                position=(event.position().x(), event.position().y()),
            )
            infra_event = InfrastructureDragDropEventData(
                domain_data=domain_event,
                mime_data=event.mimeData(),
            )
            self.drag_entered.emit(infra_event)
        else:
            event.ignore()
            if invalid_files:
    ...
    def _handle_drag_move(self, event: QDragMoveEvent,
        ...
        if files:
            ...
            self.drag_moved.emit(infra_event)
        else:
            event.ignore()
    ...
    def _handle_drop(self, event: QDropEvent,
        ...
        if valid_files:
            ...
            if valid_files:
                ...
                self.files_dropped.emit(infra_event)
            else:
                event.ignore()
        else:
            event.ignore()
...
class DragDropManager:
    ...
    def __init__(self, parent: QObject | None = None, service: DragDropService | None = None):
        # Allow injecting a shared DragDropService so signal connections and widget hooks use the same instance
        self.service = service if service is not None else DragDropService(parent)
