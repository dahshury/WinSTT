...
from src.presentation.shared.resource_helpers import (
    try_get_icon,
    try_get_pixmap,
)
...
class MainWindowUIBuilder:
    ...
    def build_visualization_renderer(self) -> "MainWindowUIBuilder":
        ...
        # Ensure the plot sits above the background strip but below controls by default
        try:
            plot_widget = self.visualization_renderer.get_plot_widget()
    ...
    def build_hardware_acceleration_indicator(self, has_acceleration: bool) -> "MainWindowUIBuilder":
        ...
        try:
            ...
            # Adjust position slightly back to the right (halfway from previous change)
            self.hw_accel_label.setGeometry(QtCore.QRect(241, 189, 135, 31))
