"""UI State Controller.

This controller manages UI state transitions and visual feedback,
separating UI concerns from business logic and using proper DDD audio visualization.
"""

from contextlib import suppress

from PyQt6.QtCore import QObject, QTimer, pyqtSignal

from src.application.audio_visualization import (
    StartVisualizationUseCase,
    StopVisualizationUseCase,
)
from src.domain.audio_visualization.entities.audio_processor import (
    AudioProcessorConfig,
)
from src.domain.audio_visualization.value_objects.visualization_settings import (
    VisualizationSettings,
)
from src.domain.audio_visualization.value_objects.waveform_data import (
    WaveformData,
)
from src.domain.common.ports.logging_port import LoggingPort
from src.infrastructure.audio_visualization.audio_processor_service import (
    AudioProcessorService,
)
from src.presentation.adapters.pyqtgraph_renderer_adapter import PyQtGraphRendererAdapter
from src.presentation.main_window.components.progress_indicator_component import (
    ProgressIndicatorComponent,
)


class UIStateController:
    """Controller for managing UI state and visual feedback."""
    
    def __init__(
        self,
        progress_indicator: ProgressIndicatorComponent,
        visualization_renderer: PyQtGraphRendererAdapter,
        start_visualization_use_case: StartVisualizationUseCase | None = None,
        stop_visualization_use_case: StopVisualizationUseCase | None = None,
        sound_player_service=None,  # Will be injected when available
        logger: LoggingPort | None = None,
    ):
        self._progress_indicator = progress_indicator
        self._visualization_renderer = visualization_renderer
        self._start_visualization_use_case = start_visualization_use_case
        self._stop_visualization_use_case = stop_visualization_use_case
        self._sound_player = sound_player_service
        self._logger = logger
        
        # UI state
        self._is_recording = False
        self._is_transcribing = False
        self._progress_timer: QTimer | None = None
        self._progress_value = 0

        # Real-time audio pipeline (infrastructure-backed)
        self._audio_processor_service: AudioProcessorService | None = None
        self._audio_processor_entity = None
        self._audio_processor_thread = None

        # Signal bridge to marshal audio updates onto the UI thread
        class _VisualizationSignalBridge(QObject):
            audio_data = pyqtSignal(object)  # numpy.ndarray

        self._viz_signal_bridge = _VisualizationSignalBridge()
        self._viz_signal_bridge.audio_data.connect(self._update_visualization_from_thread)
    
    def start_recording_ui(self, enable_sound: bool = False, sound_path: str = "") -> None:
        """Update UI for recording start using proper visualization system."""
        try:
            self._is_recording = True
            
            # Show visualizer using proper adapter
            self._visualization_renderer.show_visualization()
            
            # Start visualization using use case if available
            if self._start_visualization_use_case:
                # This would properly start the visualization system
                # For now, just show the renderer
                pass

            # Start real audio capture → waveform rendering pipeline
            if self._audio_processor_service is None:
                self._audio_processor_service = AudioProcessorService()

            if self._audio_processor_entity is None:
                config = AudioProcessorConfig(sample_rate=16000, chunk_size=1024, buffer_size=100)
                self._audio_processor_entity = self._audio_processor_service.create_processor(config)

                # Wire signals from the processor thread to our UI bridge
                self._audio_processor_thread = self._audio_processor_service.get_processor_thread(self._audio_processor_entity)
                if self._audio_processor_thread is not None:
                    # Route cross-thread updates through a Qt signal owned by the UI thread
                    with suppress(Exception):
                        self._audio_processor_thread.data_ready.connect(self._viz_signal_bridge.audio_data.emit)

            # Ensure processor is running
            if self._audio_processor_service and self._audio_processor_entity:
                try:
                    self._audio_processor_service.start_processor(self._audio_processor_entity)
                except Exception:
                    if self._logger:
                        self._logger.log_error("Failed to start audio processor for visualization")
            
            # Play start sound if enabled
            if enable_sound and sound_path and self._sound_player:
                self._sound_player.play_sound(sound_path)
            
            # Start progress animation
            self._progress_timer = QTimer()
            self._progress_timer.timeout.connect(self._animate_recording_progress)
            self._progress_timer.start(200)
            self._progress_value = 0
            
            if self._logger:
                self._logger.log_debug("Recording UI started with proper visualization")
                
        except Exception as e:
            if self._logger:
                self._logger.log_error(f"Error starting recording UI: {e}")

    def play_notification_sound(self, sound_path: str) -> None:
        """Play a one-shot notification sound without altering UI state."""
        try:
            if sound_path and self._sound_player:
                self._sound_player.play_sound(sound_path)
        except Exception as e:
            if self._logger:
                self._logger.log_error(f"Error playing notification sound: {e}")
    
    def stop_recording_ui(self) -> None:
        """Update UI for recording stop using proper visualization system."""
        try:
            self._is_recording = False
            
            # Stop progress animation
            if self._progress_timer:
                self._progress_timer.stop()
                self._progress_timer = None
            
            # Hide visualizer using proper adapter
            self._visualization_renderer.hide_visualization()
            self._visualization_renderer.clear_visualization()
            
            # Stop visualization using use case if available
            if self._stop_visualization_use_case:
                # This would properly stop the visualization system
                # For now, just hide the renderer
                pass

            # Stop and cleanup real audio processor
            try:
                if self._audio_processor_service and self._audio_processor_entity:
                    with suppress(Exception):
                        # Disconnect to avoid late signals after stop
                        if self._audio_processor_thread is not None:
                            self._audio_processor_thread.data_ready.disconnect(self._viz_signal_bridge.audio_data.emit)
                    with suppress(Exception):
                        self._audio_processor_service.stop_processor(self._audio_processor_entity)
                    with suppress(Exception):
                        self._audio_processor_service.cleanup_processor(self._audio_processor_entity)
            finally:
                self._audio_processor_thread = None
                self._audio_processor_entity = None
            
            if self._logger:
                self._logger.log_debug("Recording UI stopped with proper visualization")
                
        except Exception as e:
            if self._logger:
                self._logger.log_error(f"Error stopping recording UI: {e}")
    
    def start_transcribing_ui(self, show_progress: bool = False) -> None:
        """Update UI for transcription start."""
        try:
            self._is_transcribing = True
            
            if show_progress:
                self._progress_indicator.show_progress(0)
            
            if self._logger:
                self._logger.log_debug("Transcribing UI started")
                
        except Exception as e:
            if self._logger:
                self._logger.log_error(f"Error starting transcribing UI: {e}")
    
    def stop_transcribing_ui(self) -> None:
        """Update UI for transcription completion."""
        try:
            self._is_transcribing = False
            
            # Hide progress bar
            self._progress_indicator.hide_progress()
            
            if self._logger:
                self._logger.log_debug("Transcribing UI stopped")
                
        except Exception as e:
            if self._logger:
                self._logger.log_error(f"Error stopping transcribing UI: {e}")
    
    def update_progress(self, value: int) -> None:
        """Update progress indicator."""
        self._progress_indicator.update_progress(value)
    
    def _animate_recording_progress(self) -> None:
        """Animate the recording progress bar."""
        try:
            self._progress_value = (self._progress_value + 5) % 100
            self._progress_indicator.update_progress(self._progress_value)
        except Exception as e:
            if self._logger:
                self._logger.log_error(f"Error animating progress: {e}")
    
    def _update_visualization_from_thread(self, audio_data) -> None:
        """Render waveform safely on the UI thread from processor data."""
        try:
            # Convert numpy.ndarray to list of floats if needed
            try:
                samples_list = audio_data.tolist()
            except Exception:
                samples_list = list(audio_data)

            # Downsample for rendering performance
            if len(samples_list) > 4:
                samples_list = samples_list[::4]

            # Build WaveformData and VisualizationSettings
            waveform = WaveformData.from_samples_list(
                samples=samples_list,
                sample_rate=16000,
                timestamp_ms=0.0,
            )
            settings = VisualizationSettings.speech_optimized(width=400, height=51)

            # Render using the adapter
            self._visualization_renderer.render_waveform(waveform, settings)
        except Exception:
            # Silent failure to avoid UI spam; logging is optional
            if self._logger:
                self._logger.log_debug("Waveform render skipped due to transient data issue")
    
    @property
    def is_recording(self) -> bool:
        """Check if UI is in recording state."""
        return self._is_recording
    
    @property
    def is_transcribing(self) -> bool:
        """Check if UI is in transcribing state."""
        return self._is_transcribing
