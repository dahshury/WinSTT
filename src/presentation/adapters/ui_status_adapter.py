    ...
    def show_status(self, message: StatusMessage) -> None:
        try:
            ...
            if message.show_progress_bar and message.progress_value is not None:
                ...
                # Clamp to [0, 100]
                try:
                    val = int(max(0, min(100, int(message.progress_value))))
                except Exception:
                    val = 0
                self._progress_bar.setValue(val)
                # Also show a succinct percentage in the status label
                try:
                    pct_text = f" ({val}%)"
                    self._status_label.setText(self._status_label.text() + pct_text)
                except Exception:
                    pass
