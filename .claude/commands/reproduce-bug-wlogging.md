...
### 5. Debug with runtime evidence (Root Cause Analysis)

You have a failing test — now you need to understand **why** it fails. Do NOT guess the root cause from code reading alone. Use the `/debug-mode` skill to instrument the code and collect runtime evidence.

1. **Invoke `/debug-mode`** — this activates evidence-based debugging with structured logging and hypothesis testing.
2. **Generate 3-5 hypotheses** about the root cause based on the test failure.
3. **Instrument the crashing code path** with debug logs (following `/debug-mode` logging instructions). For Python tests, append NDJSON lines directly to the debug log file using file I/O.
4. **Run the failing test again** with instrumentation in place.
5. **Read the debug.log file** from the project root and analyze the log entries to confirm or reject each hypothesis with cited evidence.
6. **Iterate** if hypotheses are inconclusive — add more instrumentation, re-run, re-read logs. Do NOT proceed to fixing until you have log-based proof of the root cause.

**CRITICAL: The debug.log file MUST always be saved to the root directory of the project** (e.g., `E:\DL\Projects\WinSTT\debug.log`). Never save it in subdirectories, `.cursor/`, `.claude/`, or any other nested location.

**The litmus test:** Can you cite specific log lines that prove the root cause? If not, you need more instrumentation.

### 6. Write the minimal fix (Green)

Now — and only now — write the fix. Run the test. It should pass.

### 7. Verify the fix catches the bug (Critical)
...
Before claiming the bug is reproduced and fixed:
...
- [ ] `/debug-mode` was used to instrument and collect runtime evidence
- [ ] `debug.log` was saved to the **project root directory** and read for analysis
- [ ] Root cause was identified with **cited log line evidence**, not guessed from code reading
