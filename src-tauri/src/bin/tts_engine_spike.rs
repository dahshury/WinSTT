...
//
//   cargo run --release --bin tts_engine_spike -- kitten
//   cargo run --release --bin tts_engine_spike -- kitten expr-voice-2-f "custom text"
//   cargo run --release --bin tts_engine_spike -- piper
//   cargo run --release --bin tts_engine_spike -- supertonic M1
//
...
fn run_supertonic(voice: &str, text: &str) {
    let cfg = SupertonicConfig {
        cache_dir: cache_root().join("supertonic"),
    };
    let engine = SupertonicEngine::new(cfg);
    eprintln!("\n=== SUPERTONIC voice={voice} ===");
    for label in ["cold", "warm"] {
        let t = Instant::now();
        match engine.synthesize(text, voice, 1.0) {
            Ok(s) => {
                report(
                    "supertonic",
                    voice,
                ...
                if label == "cold" {
                    if let Some(io) = engine.io_report() {
                        eprintln!("  --- I/O contract ---\n{io}");
                    }
                }
            }
            Err(e) => {
...
fn main() {
    ...
    match engine {
        ...
        "supertonic" => {
            let voice = args.get(2).cloned().unwrap_or_else(|| "M1".into());
            let text = args
                .get(3)
                .cloned()
                .unwrap_or_else(|| DEFAULT_SENTENCE.into());
            run_supertonic(&voice, &text);
        }
        "chatterbox" => {
            let text = args
                .get(2)
