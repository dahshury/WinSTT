...
// end-to-end. NOT shipped — a `cargo run --bin tts_spike` harness only.
//
//   cargo run --release --bin tts_spike                       # af_heart, en-us, default sentence
//   cargo run --release --bin tts_spike -- am_michael         # pick a voice (lang inferred from catalog)
//   cargo run --release --bin tts_spike -- af_heart "custom text here"
//   cargo run --release --bin tts_spike -- --multi            # synth a representative voice per language
//   SPIKE_TTS_DEVICE=dml cargo run --release --bin tts_spike  # measure the DirectML/GPU path
//
