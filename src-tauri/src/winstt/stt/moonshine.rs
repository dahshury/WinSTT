...
impl MoonshineEngine {
	...
	pub fn load(cfg: &EngineConfig) -> SttResult<Self> {
		...
		// rather than adding it to the correctness-based `FamilyPolicy::DML_INCOMPATIBLE` mirror
		// (which tracks Electron's `_DML_INCOMPATIBLE_FAMILIES` of families that actually crash).
	...
	fn encode(&mut self, audio: &[f32]) -> SttResult<(Vec<i64>, Vec<f32>)> {
		...
		named.push((Cow::Borrowed("input_values"), SessionInputValue::from(input)));
	...
	fn decode_greedy(&mut self, enc_shape: &[i64], enc_data: &[f32]) -> SttResult<Vec<i64>> {
		...
		let (logits0, state) = self.first_decode_step(&enc_shape_usize, enc_data, &tokens, enc_frames)?;
	...
	fn first_decode_step(
		...
		let input_ids = Tensor::from_array((
			[1usize, prompt.len()],
			prompt.to_vec().into_boxed_slice(),
		))
		...
		if let Some(mask_name) = &self.decoder_enc_mask_name {
			let mask = Tensor::from_array(([1usize, enc_frames], vec![1i64; enc_frames].into_boxed_slice()))
		...
		let mut state: HashMap<String, (Vec<i64>, Vec<f32>)> = HashMap::with_capacity(self.present_output_names.len());
		for present_name in &self.present_output_names {
			...
			let (s, d) = v.try_extract_tensor::<f32>().map_err(|e| {
				SttError::Inference(format!("{present_name} extract: {e}"))
			})?;
	...
	fn past_decode_step(
		...
		if let Some(hidden_name) = &self.past_enc_hidden_name {
			...
			named.push((Cow::Owned(hidden_name.clone()), SessionInputValue::from(enc_hidden)));
		}
...
impl Transcriber for MoonshineEngine {
	...
	fn transcribe(&mut self, audio: &[f32], _opts: &TranscribeOptions) -> SttResult<Transcription> {
		...
		Ok(Transcription { text, segments: None, words: None })
	}
...
impl MoonshineTokenizer {
	fn load(tokenizer_path: &Path, tokenizer_config_path: Option<&Path>) -> SttResult<Self> {
		...
		let mut special_token_ids: std::collections::HashSet<i64> = std::collections::HashSet::new();
		...
		if let Some(added) = tok.get("added_tokens").and_then(|a| a.as_array()) {
			for entry in added {
				...
				let content = entry
					...
					.unwrap_or("")
					.to_string();
				let special = entry.get("special").and_then(|s| s.as_bool()).unwrap_or(false);
				if content == "<s>" {
					bos_id = Some(tid);
				} else if content == "</s>" {
					eos_id = Some(tid);
				...
				if let Ok(cfg) = serde_json::from_str::<serde_json::Value>(&cfg_raw) {
					if let Some(atd) = cfg.get("added_tokens_decoder").and_then(|a| a.as_object()) {
						for (tid_str, entry) in atd {
							let Ok(tid) = tid_str.parse::<i64>() else { continue };
							let content = entry
								.get("content")
								.and_then(|c| c.as_str())
								.unwrap_or("")
								.to_string();
							let special =
								entry.get("special").and_then(|s| s.as_bool()).unwrap_or(false);
	...
	fn decode_text(&self, ids: &[i64]) -> String {
		...
		text.strip_prefix(' ').map(|s| s.to_string()).unwrap_or(text)
	}
}

// ---------------------------------------------------------------------------
// Session construction + ORT helpers (mirrors whisper.rs verbatim)
// ---------------------------------------------------------------------------
...
/// Map our `Accelerator` list to ort `ExecutionProviderDispatch`es (CPU appended as fallback).
/// Identical to whisper.rs: Moonshine keeps DirectML/CUDA when requested.
...
/// Sort key for `{present|past_key_values}.N.{decoder|encoder}.{key|value}` by integer layer index
/// then sub-tensor — a canonical total order independent of graph iteration order. Works for both
/// the `present.` and `past_key_values.` prefixes (we strip whichever is present).
...
mod tests {
	...
	fn decode_maps_underscore_to_space_and_strips_leading() {
		...
		let tk = tk_with(
			&[(10, "\u{2581}And"), (11, "\u{2581}so"), (1, "<s>"), (2, "</s>")],
			&[1, 2],
		);
	...
	fn decode_byte_fallback_assembles_utf8() {
		...
		let tk = tk_with(
			&[(3, "<0xE2>"), (4, "<0x82>"), (5, "<0xAC>"), (10, "\u{2581}x")],
			&[],
		);
