...
pub(super) fn default_post_process_providers() -> Vec<PostProcessProvider> {
    let mut providers = vec![
        ...
        PostProcessProvider {
            ...
            allow_base_url_edit: false,
            models_endpoint: Some("/models".to_string()),
            // Enabled: the genai transport routes Anthropic through its NATIVE
            // Messages API (/v1/messages), which supports json_schema structured
            // output. If a structured request is ever rejected, the post-process
            // path in actions.rs falls through to legacy mode, so this is safe.
            supports_structured_output: true,
        },
        PostProcessProvider {
            id: "groq".to_string(),
...
pub(super) fn default_model_for_provider(provider_id: &str) -> String {
