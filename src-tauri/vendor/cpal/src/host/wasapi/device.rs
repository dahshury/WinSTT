...
impl Device {
    ...
    fn ensure_future_audio_client(
        ...
        // WinSTT opens microphone capture and its recording chime together.
        // Declare capture as Communications as soon as the client is activated,
        // before either GetMixFormat/IsFormatSupported or Initialize can run.
        // That keeps format discovery and stream creation in the same Windows
        // audio scenario and lets LE Audio select its bidirectional topology.
        if self.data_flow() == Audio::eCapture {
            let audio_client_2 = audio_client.cast::<Audio::IAudioClient2>()?;
            let client_properties = Audio::AudioClientProperties {
            ...
            unsafe { audio_client_2.SetClientProperties(&client_properties)? };
        }
