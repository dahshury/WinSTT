// REPRO / CHARACTERISATION harness for the `chatterbox-multilingual` degradation report.
// Independent of `cbx_probe_repro.rs` (owned by another workstream) — same mechanism, but
// adds: an 8-sentence corpus, repeated runs for determinism, a token-stream hash, an audio
// hash, a duration-plausibility number, and a word-level WER/S/D/I alignment against the
// reference text in `asr` mode.
//
//   cargo run --release --example cbx_probe_charz -- gen <model_id> <quant> <out_dir> <reps>
//   cargo run --release --example cbx_probe_charz -- asr <dir> [whisper_snapshot_dir]
//
...
fn sentence_text(key: &str) -> Option<&'static str> {
    SENTENCES.iter().find(|(k, _)| *k == key).map(|(_, t)| *t)
}
...
fn graphs_for(model_id: &str, quant: &str) -> [String; 4] {
    match model_id {
        ...
        // multilingual: only the backbone is quantized in that repo
        _ => {
...
fn run_gen(model_id: &str, quant: &str, out_dir: &Path, reps: usize) {
    ...
    println!(
        "GRAPHS model={model_id} quant={quant} dir={} {g:?}",
        dir.display()
    );
    ...
    println!(
        "SPEC model={model_id} layers={} heads={} head_dim={} kv_dtype={} embed_pos={embed_wants_pos} embed_exag={embed_wants_exag} lm_pos={lm_wants_pos} vocab={}",
        kv.past_names.len() / 2,
        kv.heads,
        kv.head_dim,
    ...
    println!(
        "REF model={model_id} ref_samples={} ref_dur_s={:.3}",
        audio.len(),
    ...
    println!(
        "COND model={model_id} cond_emb={:?} Lc={lc} prompt_tok={lp} xvec={:?} feat={:?}",
        cond_emb.shape(),
        ref_x_vector.shape(),
        prompt_feat.shape()
    );
    ...
    for (key, text) in sentences() {
        for rep in 0..reps {
            ...
            if rep == 0 {
                println!(
                    "TOK model={model_id} sent={key} prompt={prompt:?} n_ids={s}\n     pieces={:?}",
                    enc.get_tokens()
                );
            }
            ...
            let file = out_dir.join(format!("{tag}-{quant}__{key}__r{rep}.wav"));
            write_wav(&file, &out, SR);
            let (peak, rms, nan) = stats(&out);
            let mean_margin = top_margins.iter().sum::<f32>() / top_margins.len().max(1) as f32;
            // duration plausibility: 25 Hz codec => gen_tokens/25 s of NEW speech
            let gen_secs = gen_mid.len() as f32 / CODEC_HZ;
            let words = text.split_whitespace().count();
            println!(
                "GEN model={model_id} quant={quant} sent={key} rep={rep} words={words} n_text_ids={s} gen_tokens={} hit_cap={hit_cap} stop_step={stop_step} uniq_gen={} n_speech={n_speech} prompt_tok={lp} samples={} dur_s={:.3} gen_secs={gen_secs:.3} smpl_per_tok={:.1} words_per_sec={:.2} peak={peak:.3} rms={rms:.4} nan={nan} ar_ms={ar_ms} margin={mean_margin:.3} stop_rank_min={} tokhash={:016x} audhash={:016x} file={}",
                gen_mid.len(),
                uniq.len(),
                out.len(),
                out.len() as f32 / SR as f32,
                out.len() as f32 / n_speech.max(1) as f32,
                words as f32 / (out.len() as f32 / SR as f32).max(0.001),
            ...
            println!(
                "TOKENS sent={key} rep={rep} head={:?} tail={:?}",
...
/// Word-level Levenshtein with backtrace -> (sub, del, ins, ref_len).
fn wer(reference: &[String], hyp: &[String]) -> (usize, usize, usize, usize) {
    ...
    // In-flight work from a concurrent session; this `allow` is the minimal
    // change that gets `cargo clippy --all-targets` green without touching the
    // algorithm. Drop it when that session rewrites this loop.
...
fn run_asr(dir: &Path, snap: &str) {
    ...
    for p in paths {
        ...
        let reference = sentence_text(&key).unwrap_or("");
...
fn main() {
    ...
    match mode {
        "gen" => {
            ...
            let out_dir = PathBuf::from(args.get(4).cloned().unwrap_or_else(|| "cbx_charz".into()));
            let reps: usize = args.get(5).and_then(|s| s.parse().ok()).unwrap_or(1);
            run_gen(&model_id, &quant, &out_dir, reps);
        }
        "asr" => {
            let dir = PathBuf::from(args.get(2).cloned().unwrap_or_else(|| "cbx_charz".into()));
            let snap = args
                .get(3)
                .cloned()
