...
function renderSwitch(args: Partial<Args> = {}) {
	const onModelChange = args.onModelChange ?? mock<(id: string) => void>(() => undefined);
	const onConfigureCloud = args.onConfigureCloud ?? mock<() => void>(() => undefined);
	const pickLocalDefault = args.pickLocalDefault ?? mock<() => string | null>(() => "tiny");
...
describe("useSttSourceSwitch", () => {
	...
	test("flipping to Local leaves the model untouched when there is no local candidate", () => {
		setKeys("sk-openai", "");
