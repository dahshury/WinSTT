...
type Update = ReturnType<typeof useSettingsStore.getState>["updateModelSettings"];
...
function stateEntry(estimatedBytes: number, cached: boolean): ModelStateEntry {
	return {
		cache: { state: cached ? "cached" : "not_cached", progress: cached ? 1 : 0 },
...
// A typical valid catalog: "tiny" is smallest+realtime-capable, "large" is big.
const CATALOG: ModelInfo[] = [
	model({ id: "tiny", backend: "onnx_asr", supportsRealtime: true }),
	model({ id: "base", backend: "faster_whisper", supportsRealtime: true }),
	model({ id: "large", backend: "onnx_asr", supportsRealtime: false }),
];
...
describe("useStaleModelFallback", () => {
	...
	test("does NOT patch when the resolved fallback equals the (stale-by-empty) current model", () => {
		...
		renderFallback({
			...
			currentMainModel: "",
			currentRealtimeModel: "x", // not realtime-viable -> see realtime test below
			update,
		});
		// next ("") === currentMainModel ("") → main effect must not patch.
		// Realtime: realtimeViable is empty (supportsRealtime=false) →
		// needsModelFallback("x", []) is true, pickDefault over empty → null →
		// no patch. So NO update at all.
		expect(update).not.toHaveBeenCalled();
	});
	...
	test("does NOT patch the main model when no fallback entry has a backend", () => {
		...
		const noBackend: ModelInfo[] = [
			{ ...model({ id: "tiny" }), backend: "" as unknown as ModelInfo["backend"] },
		];
		renderFallback({
			catalogLoaded: true,
	...
	test("falls back the realtime model to the smallest realtime-viable id", () => {
		const update = mock<Update>(() => undefined);
		renderFallback({
			catalogLoaded: true,
			catalogModels: CATALOG,
			...
			// (smallest cached supportsRealtime model).
			currentMainModel: "base",
	...
	test("ignores a stale realtime id when only a non-realtime model could be picked", () => {
		const update = mock<Update>(() => undefined);
		// Only a non-realtime model exists. realtimeViable = [] →
		// needsModelFallback("stale", []) true, pickDefault(realtime filter)
		// over a catalog with no realtime models → null → no realtime patch.
		const onlyLarge: ModelInfo[] = [model({ id: "large", supportsRealtime: false })];
		renderFallback({
			catalogLoaded: true,
	...
	test("leaves a valid realtime model alone", () => {
		...
		renderFallback({
			...
			statesById: STATES,
			currentMainModel: "base",
			currentRealtimeModel: "base", // realtime-viable and present
			update,
		});
		expect(update).not.toHaveBeenCalled();
	});
	...
	test("does NOT patch realtime when the resolved pick equals the current realtime model", () => {
		...
		const update = mock<Update>(() => undefined);
		const emptyIdRealtime: ModelInfo[] = [model({ id: "", supportsRealtime: true })];
		renderFallback({
			catalogLoaded: true,
