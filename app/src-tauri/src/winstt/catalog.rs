...
pub fn resolve_quantization(
	...
	if quant.is_empty() || quant == "auto" {
		...
		if !accel.is_cuda() && family.prefers_int8_off_cuda() && publishes("int8") {
			return Some("int8");
		}
		return None;
	}
...
mod tests {
	...
	fn catalog_total_count_is_42() {
		assert_eq!(STT_CATALOG.len(), 42, "catalog.json ships exactly 42 STT models");
	}
	...
	fn per_family_counts_match_catalog_json() {
		...
		assert_eq!(count(Family::Custom), 0, "custom never appears in the shipped catalog");
	...
	fn every_model_has_a_repo_and_at_least_one_quant() {
		for m in STT_CATALOG {
			assert!(!m.onnx_model_name.is_empty(), "{} missing onnx_model_name", m.id);
			let quants = quantizations_for_id(m.id);
			assert!(!quants.is_empty(), "{} must publish at least one quant", m.id);
		}
	}
	...
	fn dml_incompatible_equals_int8_preferred() {
		...
		assert_eq!(dml, int8, "the two family lists must be byte-identical sets");
		assert_eq!(dml, expected, "the set must equal the canonical 7-family list");
		assert_eq!(dml.len(), 7, "exactly 7 families are DML-incompatible / int8-preferred");
	...
	fn auto_on_cuda_picks_fp16_only_for_large_models_that_publish_it() {
		...
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cuda, turbo.param_count, Some(turbo.available_quantizations), turbo.family),
			Some("fp16")
		);
		// tiny: 37M, publishes fp16 but is BELOW the 500M floor -> fp32 (None).
		let tiny = find("tiny").unwrap();
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cuda, tiny.param_count, Some(tiny.available_quantizations), tiny.family),
			None
		);
		// large-v3: 1.55B but publishes ONLY "" (no fp16) -> fp32 (None) even though huge.
		let lv3 = find("large-v3").unwrap();
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cuda, lv3.param_count, Some(lv3.available_quantizations), lv3.family),
			None
		);
	}
	...
	fn auto_off_cuda_picks_int8_for_int8_preferred_families() {
		...
		assert_eq!(
			resolve_quantization("auto", Accelerator::DirectMl, cohere.param_count, Some(cohere.available_quantizations), cohere.family),
			None,
			"cohere doesn't publish int8 -> auto must not invent it"
		);
		// NeMo Canary 1B v2 on DirectML -> int8 (publishes int8).
		let canary = find("nemo-canary-1b-v2").unwrap();
		assert_eq!(
			resolve_quantization("auto", Accelerator::DirectMl, canary.param_count, Some(canary.available_quantizations), canary.family),
			Some("int8")
		);
		// Same on plain CPU.
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cpu, canary.param_count, Some(canary.available_quantizations), canary.family),
			Some("int8")
		);
		// GigaAM CTC on CPU -> int8.
		let giga = find("gigaam-v3-e2e-ctc").unwrap();
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cpu, giga.param_count, Some(giga.available_quantizations), giga.family),
			Some("int8")
		);
	}
	...
	fn auto_off_cuda_does_not_int8_whisper_or_moonshine() {
		...
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cpu, small_en.param_count, Some(small_en.available_quantizations), small_en.family),
			None
		);
		// Moonshine on DirectML -> fp32 even though it publishes int8 (not int8-preferred family).
		let moon = find("moonshine-base").unwrap();
		assert_eq!(
			resolve_quantization("auto", Accelerator::DirectMl, moon.param_count, Some(moon.available_quantizations), moon.family),
			None
		);
	}
	...
	fn concrete_quant_not_published_falls_back_to_fp32() {
		...
		let tiny = find("tiny").unwrap();
		assert_eq!(
			resolve_quantization("int8", Accelerator::Cpu, tiny.param_count, Some(tiny.available_quantizations), tiny.family),
			None
		);
	}
	...
	fn concrete_sub_fp16_on_cuda_falls_back_to_fp32() {
		...
		let moon = find("moonshine-base").unwrap();
		assert_eq!(
			resolve_quantization("int8", Accelerator::Cuda, moon.param_count, Some(moon.available_quantizations), moon.family),
			None
		);
		// q4 on CUDA for whisper -> fp32.
		let tiny = find("tiny").unwrap();
		assert_eq!(
			resolve_quantization("q4", Accelerator::Cuda, tiny.param_count, Some(tiny.available_quantizations), tiny.family),
			None
		);
	}
	...
	fn concrete_quant_passes_through_off_cuda_and_for_fp16_on_cuda() {
		...
		let moon = find("moonshine-base").unwrap();
		assert_eq!(
			resolve_quantization("int8", Accelerator::Cpu, moon.param_count, Some(moon.available_quantizations), moon.family),
			Some("int8")
		);
		// fp16 on CUDA -> fp16 (in GPU_COMPATIBLE set; hits the decoder repair path).
		let turbo = find("large-v3-turbo").unwrap();
		assert_eq!(
			resolve_quantization("fp16", Accelerator::Cuda, turbo.param_count, Some(turbo.available_quantizations), turbo.family),
			Some("fp16")
		);
	}
	...
	fn effective_quantization_collapses_none_to_empty_string() {
		let tiny = find("tiny").unwrap();
		assert_eq!(
			effective_quantization("auto", Accelerator::Cpu, tiny.param_count, Some(tiny.available_quantizations), tiny.family),
			""
		);
		let turbo = find("large-v3-turbo").unwrap();
		assert_eq!(
			effective_quantization("auto", Accelerator::Cuda, turbo.param_count, Some(turbo.available_quantizations), turbo.family),
			"fp16"
		);
	}
	...
	fn off_catalog_repo_is_permissive_about_quants() {
		...
		assert_eq!(
			resolve_quantization("auto", Accelerator::Cuda, 600_000_000, None, Family::Whisper),
			Some("fp16")
		);
	}
	...
	fn picker_filters_sub_fp16_only_on_cuda() {
		...
		assert_eq!(picker_quantizations_for(moon, Accelerator::Cuda), vec!["", "fp16"]);
