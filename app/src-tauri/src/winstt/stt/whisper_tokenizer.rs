...
impl WhisperTokenizer {
	...
	pub fn from_tokens(tokens: HashMap<String, i64>) -> SttResult<Self> {
		...
		let translate_token_id = tokens.get("<|translate|>").copied().unwrap_or(transcribe_token_id);
	...
	pub fn is_special(&self, id: i64) -> bool {
		self.vocab.get(&id).map(|t| t.starts_with("<|")).unwrap_or(false)
	}
	...
	pub fn vocab_size(&self) -> i64 {
		self.tokens.values().copied().max().map(|m| m + 1).unwrap_or(0)
	}
}
